...
describe('listProjects', () => {
  ...
  it('песочницы панели в список проектов не попадают', () => {
    const sandboxCwd = join(homedir(), '.agentdeck', 'chats', 'new-123');
  ...
  it('устаревшая песочница под другим корнем тоже отсеивается', () => {
    ...
    // но по устойчивому признаку agentdeck/chats/new- он всё равно отсеивается.
    writeTranscript('enc-old-sb', 'osb', 'C:/Users/x/.claude/agentdeck/chats/new-123');
    expect(listProjects(projectsDir)).toEqual([]);
  });
