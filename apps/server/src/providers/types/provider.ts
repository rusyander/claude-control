...
/**
 * Механизмы, которые провайдер ведёт СВОЕЙ моделью, а не универсальным адаптером
 * (`*Config` выше). Заведено ради матрицы верности (П1.1): у Claude скиллы,
 * команды, субагенты, хуки, права, MCP и переменные живут на собственных богатых
 * маршрутах панели, поэтому `skillsConfig`/`hooksConfig`/… у него нет вовсе — и
 * без этого блока матрица посчитала бы, что переносить в Claude нечего.
 *
 * Это НЕ дубликат `capabilities`: там статус раздела панели, здесь — факты,
 * которых в статусе нет (какие события отдаёт механизм хуков и какие из них
 * умеют блокировать, правилами или режимом выражены права, есть ли понятие
 * субагента вообще). Одиннадцатый CLI со своей моделью заполняет этот блок в
 * своём файле каталога, и ни одного файла домена при этом не правит (§5.4).
 */
export interface ProviderNativeMechanisms {
  /** События собственного механизма хуков и способность каждого остановить действие. */
  hookEvents?: readonly { name: string; blocking: boolean }[];
  /** Чем выражены права: правилами поимённо или режимом на весь CLI. */
  permissions?: { model: 'rules' | 'mode'; decisions: readonly ('allow' | 'ask' | 'deny')[] };
  /** Есть ли у CLI понятие скилла (каталог со `SKILL.md` либо своя модель). */
  skills?: boolean;
  /** Есть ли у CLI слэш-команды. */
  commands?: boolean;
  /** Есть ли у CLI субагенты — единственный вид записи, у которого нет `*Config` ни у кого. */
  subagents?: boolean;
  /** Есть ли у CLI MCP-серверы. */
  mcp?: boolean;
  /** Есть ли у CLI переменные окружения своей настройкой. */
  env?: boolean;
  /** Есть ли у CLI плагины как единица. */
  plugins?: boolean;
}
...
export interface ConfigProvider {
  ...
  /**
   * Разделы, которые провайдер ведёт СОБСТВЕННОЙ моделью (см.
   * `ProviderNativeMechanisms`). Не задан → все механизмы провайдера описаны
   * универсальными адаптерами `*Config`, и матрица считает по ним.
   */
  nativeMechanisms?: ProviderNativeMechanisms;
  /**
   ...
   * подбора (fail-closed, и это не временный пробел, а решение):
   *
   * 1. способ передать модель на один прогон ЗАДОКУМЕНТИРОВАН и реализован в
   *    `assistant.oneShotArgs` (codex `-m`, gemini `-m`);
   * 2. семейства однородны по силе — «свежайшая в семействе» обязана означать
   *    ту же ступень, а не другую. У Qwen семейство `qwen` мешает `flash`,
   *    `plus` и `max` в одну кучу: понижение по нему завтра стало бы
   *    ПОВЫШЕНИЕМ на `max`, молча;
   * 3. семейства одного вендора — выбирать за пользователя ВЕНДОРА панель не
   *    станет. Поэтому его нет у OpenCode (шлюз ко всем сразу) и у Aider
   *    (оболочка поверх любой модели, `modelVendors` у него нет вовсе).
   *
   * Верх лестницы — не «потолок разговора»: потолком у чужого CLI остаётся его
   * СОБСТВЕННАЯ настроенная модель, которой панель не знает и не трогает. Класс
   * работы, которому положен потолок (`design`, `investigation`, `review`),
