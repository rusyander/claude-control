...
- **Команды** — всё, что вызывается через `/`, одним списком с поиском: скиллы, файлы команд, плагины и встроенные команды CLI, с описанием, владельцем и переходом к правке
...
- **Git проекта** — текущая ветка, список изменённых файлов, переключение, создание ветки, коммит и `pull` прямо из вкладки; раздел появляется, только если в проекте есть `.git`
...
- **Ветка, файлы, коммит и pull там же,** если в проекте есть `.git` (нет — раздела нет вовсе): на кнопке текущая ветка, число изменённых файлов и отставание от удалённого, а под ней — сам список изменённых файлов, локальные ветки для переключения, ряд `pull`, поле новой ветки и поле сообщения коммита. `pull` тянет в текущую ветку по её upstream либо из выбранной ветки удалённого; это единственная операция, которая ходит в сеть и может привести к слиянию — конфликт панель не разбирает и не откатывает, его текст уходит вам как есть. Никакого `push` и удаления веток — осознанно: это не клиент git, это то, что нужно, пока агент работает в репозитории. Внутри — `git` без оболочки, имена веток проверяет сам `git check-ref-format`, а переключиться и потянуть можно только по имени из показанного списка, чтобы случайный ref не увёл HEAD в detached.
...
Claude помечен **verified**: его путь пройден вживую и покрыт тестами. Остальные — **experimental**: форматы взяты из документации каждого CLI и закрыты round-trip-тестами, но первую реальную запись чужого формата стоит проверить глазами. Именно поэтому вокруг чужих форматов выстроены четыре инструмента — предпросмотр записи, проверка провайдера на вашей машине, ежесуточная сверка с опубликованными схемами и перенос настроек между CLI. Как каждый устроен и почему именно так — [Провайдеры: инструменты панели](docs/PROVIDER-TOOLS.ru.md).

Насколько это не теория: первый же прогон сверки нашёл настоящий дрейф — `experimental.hook` исчез из схемы OpenCode, и раздел хуков там стал только на чтение по факту, а не по догадке.
...
Введённые ключи лежат зашифрованными (AES-256-GCM) в `agentdeck/provider-keys.enc`, фраза — машинно-локальный файл с правами `0600`. Наружу отдаётся только маска `sk-…1234`. В логи, копии, историю, поиск и архив переноса окружения ключи не попадают — на это есть тест, перебирающий всех провайдеров.
...
- **Форматы — из документации, а не из головы.** Незадокументированного нет: ни формата, ни переопределения каталога (уважаются `CODEX_HOME`, `XDG_CONFIG_HOME` и `OPENCODE_CONFIG` у OpenCode, `QWEN_HOME`).
...
**Ключи не попадают в git.** Сухой прогон того коммита, который сделал бы участник проекта: `git add -An --all` берёт только исходники, история чистая, а само приложение внутрь репозитория не пишет вообще — все пути записи ведут в `~/.claude` или `~/.agentdeck`. Следствие: вложения чата лежат в папке панели, а не в рабочем дереве, и `git add -A` их не заберёт.
...
[MIT](LICENSE) — можно использовать, изменять и распространять как угодно, в том числе в
коммерческих продуктах; достаточно сохранить строку копирайта. Гарантий нет.
