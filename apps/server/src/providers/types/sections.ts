...
export interface ProviderPermissionsConfigLocation {
  ...
  format:
    ...
    | 'cursor-json';
  /**
   * ЧТО формат способен выразить — факт из документации CLI, а не вкус:
   *
   * - `rules` — ПРАВИЛА поимённо (списки `allow`/`ask`/`deny`, массив таблиц
   *   `[[permission.rules]]`): правило источника едет правилом;
   * - `mode` — только режим-переключатель на весь CLI (`approval_policy` у Codex,
   *   `GOOSE_MODE` у Goose): отдельное правило записать НЕКУДА.
   *
   * Поле обязательное и читает его матрица верности (П1.1): без него уровень
   * правила пришлось бы брать из таблицы «провайдер → уровень», а она и есть то,
   * что начинает лгать на одиннадцатом CLI.
   */
  model: 'rules' | 'mode';
  /**
   * Решения, которые формат принимает. Заполняется по документации: у Cursor
   * списка `ask` нет вовсе, у Goose решение вообще одно на весь CLI. Правило с
   * решением не из этого списка переносится ПОНИЖЕНИЕМ В СТОРОНУ СТРОГОСТИ
   * (`ask` → `deny`), и матрица обязана сказать об этом вслух.
   */
  decisions: readonly ('allow' | 'ask' | 'deny')[];
  ...
  /**
   * СЛОВАРЬ ПРАВИЛ цели: чем её грамматика отличается от канонической
   * (`Bash(git push:*)`) — факт из документации CLI, читает его перевод прав
   * (П2.2, `domains/portability/permissions-map.ts`).
   *
   * Поля нет — правило едет строкой как есть. Это не небрежность, а решение
   * владельца (20.09.2026): у kimi грамматика правила claude-совместима по его
   * же документации, а у cursor/continue/gemini/qwen словарь инструментов панель
   * не читала, и СОЧИНИТЬ соответствие хуже, чем довезти строку и сказать, что
   * имя инструмента словарём цели не подтверждено.
   */
  ruleGrammar?: ProviderPermissionRuleGrammar;
}

/**
 * Грамматика правил ОДНОЙ цели там, где она задокументирована и отличается от
 * канона. Сегодня такой один — OpenCode: словарь у него закрытый и строчными.
 */
export interface ProviderPermissionRuleGrammar {
  /** Имя инструмента канона → имя у цели. Инструмента нет в карте — имя не меняется. */
  tools: Readonly<Record<string, string>>;
  /**
   * Словарь ЗАКРЫТ: цель знает ровно эти инструменты, и правило о любом другом
   * ей записать нельзя. Открытый словарь (карта как перевод известных имён, а
   * прочие по значению) — `false`.
   */
  closed: boolean;
  /**
   * Инструменты ЦЕЛИ, у которых её формат принимает уточнение аргумента. У
   * OpenCode карта шаблонов задокументирована только для `bash`; правило вида
   * `Edit(src/**)` там записать нечем, и это отказ, а не запись без аргумента:
   * `edit: "deny"` запретил бы ВСЕ правки вместо одного каталога.
   */
  argumentTools: readonly string[];
  /**
   * Синтаксис самого УТОЧНЕНИЯ у цели: канонический или свой.
   *
   * Канон пишет аргумент грамматикой Claude — `git push:*` значит «команда
   * начинается с `git push`». У OpenCode на том же месте КАРТА ШАБЛОНОВ команд,
   * и `git push:*` не совпадает там ни с одной настоящей командой. Переписать
   * одну грамматику в другую нечем: соответствия нет в документации OpenCode, а
   * выдумать его запрещает инвариант 2. Поэтому `own` значит «правило с
   * аргументом здесь не пишется вовсе» — отказ с причиной (инварианты 3 и 4).
   * Дословная запись выглядела бы успехом и не запрещала ничего: молча снятый
   * запрет, то есть нарушение инварианта 6.
   */
  argumentSyntax: 'canon' | 'own';
  /**
   * У цели инструмент описывается ЛИБО общим уровнем, ЛИБО картой шаблонов, но
   * не двумя формами сразу (карта OpenCode — по ключу на инструмент). Правило с
   * аргументом к инструменту, у которого уже стоит общий уровень, такой формат
   * выразить не может: заменить уровень картой значило бы снять запрет со всего
   * остального, а дописать рядом — некуда. Выбор человеческий.
   */
  oneShapePerTool: boolean;
}
...
export interface ProviderHooksConfigLocation {
  ...
  path: (override?: string) => string;
  /**
   * ВСЕ задокументированные события этого механизма — именами самого CLI.
   *
   * Список ведёт адаптер формата (`lib/qwen-hook.ts`, `lib/kimi-hook.ts`), а
   * каталог на него ссылается: второй копии имён не существует. Поле живёт
   * ЗДЕСЬ, а не в матрице верности, ровно по правилу §5.4 — одиннадцатый CLI
   * получает полный столбец матрицы, добавив запись в каталог и не тронув ни
   * одного файла домена. Пока список лежал в `domains/portability/fidelity.ts`,
   * механизм хуков нового CLI был домену не виден, и его событие объявлялось
   * отсутствующим у цели, которая его поддерживает.
   *
   * События, которых канон не знает (`file_edited` у OpenCode), перечисляются
   * наравне с остальными: их отбрасывает отображение в триггеры канона, а
   * умолчание здесь означало бы «механизм без событий» — другое утверждение.
   */
  events: readonly string[];
  /**
   * События, которые у ЭТОГО CLI умеют ОСТАНОВИТЬ действие (код выхода 2), — по
   * его документации. Остальные события механизма наблюдательные.
   *
   * Список задаётся, а не выводится из имени события: у Kimi блокировать умеют
   * ровно три события из четырнадцати, и матрица верности обязана это знать —
   * иначе блокирующий хук уехал бы наблюдателем, то есть запрет снялся бы молча
   * (инвариант 6). Там, где документация блокировку не называет, событие здесь
   * не перечисляется: недообещать у цели — сторона строгости.
   */
  blockingEvents: readonly string[];
  /**
...
export interface ProviderPluginsConfigLocation {
  ...
  /**
   * Раздел ТОЛЬКО ЧИТАЕТСЯ — текст причины, её видит человек (тот же приём, что
   * у `ProviderHooksConfigLocation.writeDisabledReason`). У Kimi состоянием
   * плагинов владеет его собственная команда `/plugins`, а форма реестра не
   * задокументирована: плагин как ЕДИНИЦА туда не переносится, и матрица
