 ...
 *    паспорта среды. Такого решения панель пока не спрашивает, и ВЫЗЫВАЮЩЕГО У
 *    ЭТОГО МОДУЛЯ НЕТ: `probeHookNeeds` не зовёт ни маршрут, ни действие, ни
 *    тест, а `ObservedNeeds` приезжает в импортёры только из подстановки в
 *    тестах. Наблюдательная половина П0.2 снята с тикета 19.09.2026 и отложена
 *    до волны П2, где у панели появится подтверждение на КАЖДЫЙ скрипт. Пока
 *    его нет, ни одна запись в паспорте не может быть помечена `observed`;
