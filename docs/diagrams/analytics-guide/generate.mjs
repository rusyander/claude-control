/**
 * Схема путеводителя «Аналитика». Источник — этот генератор, `.drawio` — его
 * вывод:
 *
 *   node docs/diagrams/analytics-guide/generate.mjs <абсолютный путь>.drawio
 *
 ...
 *
 * Страница одна, и она про происхождение чисел. На экране стоят токены, доллары
 * и «идёт сейчас»; снимок не может показать, что первые посчитаны обходом
 * журналов Claude Code, вторые — расчёт по прайсу, которого при подписке никто
 * не выставляет, а третье вообще не из файлов, а из списка процессов машины.
 */
...
// ─── Страница: как получается отчёт ────────────────────────────────────────

const p1 = newPage('Как получается отчёт', 'p1');
heading(
  p1,
  60,
  30,
  'Как получается отчёт и почему деньги здесь — оценка',
  'Токены читаются из журналов самого Claude Code, стоимость считается по прайсу, а «работает сейчас» берётся из списка процессов.',
  1240,
);
...
const files = card(p1, 'a:files', AX, 150, AW, {
  pal: C.store,
  icon: 'shape=note;size=14',
  iconText: 'jsonl',
  title: 'Транскрипты Claude Code',
  lines: [
    '~/.claude/projects/<проект>/<сессия>.jsonl — по строке на событие разговора. Их пишет сам CLI, панель только читает.',
    'Нет транскриптов — нет и отчёта: считать не из чего, и это не поломка.',
  ],
});

const period = card(p1, 'a:period', AX, files.y + files.h + 55, AW, {
  pal: C.human,
  icon: 'shape=card',
  iconText: 'период',
  title: 'Период выбираете вы',
  lines: [
    'Сегодня, 7 дней, 30 дней или свой диапазон. Границы — КАЛЕНДАРНЫЕ местные сутки: «7 дней» это сегодня и шесть предыдущих, а не 168 часов назад.',
  ],
});

card(p1, 'a:live', AX, period.y + period.h + 55, AW, {
  pal: C.cli,
  icon: 'shape=process',
  iconText: 'ps',
  title: 'Живой срез — другой источник',
  lines: [
    'Работающие агенты берутся из списка процессов операционной системы, а не из файлов, и обновляются сами.',
    'Нет прав на перечисление процессов — раздел просто пуст.',
  ],
});

const pick = card(p1, 'a:pick', BX, 150, BW, {
  pal: C.check,
  icon: 'shape=hexagon',
  iconText: 'отбор',
  title: 'Какие файлы вообще открываются',
  lines: [
    'Файл, не менявшийся с начала периода, не читается: свежих записей в нём быть не может.',
    'Отобранные читаются построчно потоком — среди них есть и на десятки мегабайт. Недописанная последняя строка живой сессии просто пропускается.',
  ],
});

const count = card(p1, 'a:count', BX, pick.y + pick.h + 55, BW, {
  pal: C.check,
  icon: 'shape=hexagon',
  iconText: '1 ответ',
  title: 'Как считается один ответ',
  lines: [
    'Claude Code пишет отдельную строку на каждый блок ответа и повторяет в них расход целиком, дописывая выход по мере генерации.',
    'Панель берёт самую полную строку и учитывает ответ ОДИН раз — иначе выход занижался бы, а запросы считались бы по нескольку раз.',
    'Проект сессии — каталог, из которого её запустили (cwd первой строки файла), а не тот, куда агент перешёл внутри разговора.',
  ],
});

card(p1, 'a:price', BX, count.y + count.h + 55, BW, {
  pal: C.check,
  icon: 'shape=hexagon',
  iconText: '$',
  title: 'Откуда берётся цена',
  lines: [
    'Тарифы подтягиваются с сайта Anthropic; сети нет — работает встроенная таблица, и она может отставать от прайса.',
    'Своя цена, вписанная в настройках, побеждает обе и применяется как есть.',
  ],
});

const report = card(p1, 'a:report', CX, 150, CW, {
  pal: C.panel,
  title: 'Отчёт',
  lines: [
    'Итоги периода, ряд по дням, разбивки по моделям, проектам, часам суток, инструментам и сессиям.',
    'Полный обход стоит секунд, поэтому результат держится в памяти около минуты: переключение фильтров не перечитывает тысячу файлов заново.',
    'Внизу честно написано, сколько файлов просмотрено и за сколько миллисекунд.',
  ],
});

const now = card(p1, 'a:now', CX, report.y + report.h + 55, CW, {
  pal: C.panel,
  title: '«Работает сейчас» и «активных сессий»',
  lines: [
    'Это два разных факта. Работающие агенты — процессы; активная сессия — транскрипт, менявшийся за последние десять минут.',
    'Первое обновляется само каждые пять секунд, второе пересчитывается вместе с отчётом.',
  ],
});

card(p1, 'a:money', CX, now.y + now.h + 55, CW, {
  pal: C.warn,
  title: 'Деньги — оценка, а не счёт',
  lines: [
    'При подписке за токены не списывают ничего: сумма отвечает на вопрос «столько же работы через API стоило бы вот столько».',
    'Лимиты подписки считаются иначе и в этих числах не видны — панель об этом прямо пишет под таблицей.',
  ],
});

link(p1, 'a:files', 'a:pick', 'весь каталог projects', {
  colour: FLOW.data,
  exit: [1, 0.3],
  entry: [0, 0.3],
});
link(p1, 'a:period', 'a:pick', 'границы периода', {
  colour: FLOW.human,
  exit: [1, 0.3],
  entry: [0, 0.8],
});
link(p1, 'a:pick', 'a:count', 'отобранные строки', {
  colour: FLOW.fwd,
  exit: [0.5, 1],
  entry: [0.5, 0],
});
link(p1, 'a:count', 'a:price', 'токены по моделям', {
  colour: FLOW.fwd,
  exit: [0.5, 1],
  entry: [0.5, 0],
});
link(p1, 'a:count', 'a:report', 'разрезы', {
  colour: FLOW.fwd,
  exit: [1, 0.3],
  entry: [0, 0.4],
});
link(p1, 'a:price', 'a:report', 'суммы рядом с токенами', {
  colour: FLOW.fwd,
  exit: [1, 0.3],
  entry: [0, 0.9],
});
link(p1, 'a:live', 'a:now', 'раз в пять секунд', {
  colour: FLOW.fwd,
  dashed: true,
  exit: [1, 0.5],
  entry: [0, 0.5],
...
legend(p1, 60, bottomOf(p1) + 60, [
  ...
  { caption: 'процессы машины', fill: C.cli.tint, stroke: C.cli.line },
  ...
  { caption: 'оговорка', fill: C.warn.tint, stroke: C.warn.line },
  ...
  { caption: 'обновляется само', edgeColour: FLOW.fwd, dashed: true },
]);
