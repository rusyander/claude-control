...
describe('маршруты групп: переменные окружения и краевые случаи', () => {
  ...
  it('повторный PUT без изменения env не переписывает settings.json заново', async () => {
    const backupDir = join(root, 'agentdeck', 'backups');
  ...
  /**
   * НАХОДКА (major, latent): applyEntityState не имеет ветки для kind==='permission'.
   * Право может быть участником группы (entityRefSchema включает 'permission',
   * readPermissions отдаёт для прав groupIds, а GroupFormModal даже подсвечивает
   * конфликт двух прав с одним шаблоном) — но переключение группы для права
   * НИЧЕГО не пишет в settings.json: ставится только отметка disabledByGroup.
   * Через UI право сейчас в группу не добавить (MemberPicker его не предлагает),
   * но состав группы приходит и из importState — тогда баг достижим.
   *
   * Тест ждёт правильного поведения (право убирается из settings.json при
   * выключении группы) — сейчас оно НЕ реализовано. Снять skip после добавления
   * ветки permission в applyEntityState (убрать/вернуть паттерн в списке решения).
   */
  it('БАГ: выключение группы с правом-участником должно снимать право из settings.json', async () => {
    writeFileSync(
      settingsPath(),
