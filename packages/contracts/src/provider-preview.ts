...
export interface ProviderPreviewResponse {
  ...
  truncated: boolean;
  /**
   * Хеши «было» и «станет» — ТОЛЬКО у обрезанного сравнения (`truncated`).
   *
   * Показать их человеку нечего, и они не для показа: у необрезанного диффа
   * содержимое файла целиком лежит в `lines`, и отпечаток показанного плана
   * ловит любую правку файла между предпросмотром и записью. У обрезанного не
   * лежит НИЧЕГО — ни строк, ни счётчиков (`added`/`removed` там нули), — и
   * сторож устаревшего плана слеп ровно к тем файлам, которые велики настолько,
   * что править их вручную опаснее всего.
   */
