/**
 * Схема путеводителя «Интеграции». Источник — этот генератор, `.drawio` — его вывод:
 *
 *   node docs/diagrams/integrations-guide/generate.mjs <абсолютный путь>.drawio
 *
 ...
 *
 * Страница одна. Главное, что она объясняет: наружу ходит ТОЛЬКО сервер панели —
 * ни браузер, ни агент: токен лежит в зашифрованном хранилище и в ответ, промпт
 * или конфиг MCP не попадает.
 */
...
// ─── Страница: три направления одной вкладки ───────────────────────────────

const p1 = newPage('Кто кому звонит', 'p1');
heading(
  p1,
  60,
  30,
  'Кто кому звонит',
  'Наружу ходит сервер панели. Браузер и агент — никогда: у них нет токена, и получить его им неоткуда.',
  1240,
);
...
const human = card(p1, 'i:human', AX, 150, AW, {
  pal: C.human,
  icon: 'shape=card',
  iconText: 'вы',
  title: 'Вы в панели',
  lines: [
    'Заполняете карточку, нажимаете «Проверить связь», заводите дефект, публикуете отчёт. Каждое действие — явное нажатие.',
  ],
});

const agent = card(p1, 'i:agent', AX, human.y + human.h + 55, AW, {
  pal: C.cli,
  icon: 'shape=process',
  iconText: 'агент',
  title: 'Агент в разговоре',
  lines: [
    'Получает доступ отдельной кнопкой «Подключить MCP Atlassian»: в разделе MCP появляется сервер-прокси, который ходит в ЭТУ ЖЕ панель. Ключа у агента нет.',
  ],
});

card(p1, 'i:token', AX, agent.y + agent.h + 55, AW, {
  pal: C.store,
  icon: 'shape=note;size=14',
  iconText: 'enc',
  title: 'Токены',
  lines: [
    'agentdeck/provider-keys.enc, ключи int: плюс идентификатор карточки. Наружу — ни в ответе, ни в журнале, ни в промпте, ни в конфиге MCP: только маска вида abc…4f21.',
  ],
});

const server = card(p1, 'i:server', BX, 150, BW, {
  pal: C.panel,
  icon: 'shape=hexagon',
  iconText: 'сервер',
  title: 'Сервер панели — единственная дверь наружу',
  lines: [
    'Берёт токен из хранилища, собирает заголовок и делает запрос. Диалект Atlassian (облако или Server/DC) определяется живой пробой и запоминается.',
    'Выключенная или незаполненная карточка отвечает 404 «не подключена» — до всякого обращения в сеть.',
  ],
});

const read = card(p1, 'i:read', BX, server.y + server.h + 55, BW, {
  pal: C.check,
  icon: 'shape=hexagon',
  iconText: 'чтение',
  title: 'Чтение — когда открыт экран',
  lines: [
    'Поиск задач, страница требований, состояние дефекта. Фоновых опросов и локальной копии задач нет: запрос уходит по открытию экрана или по кнопке.',
  ],
});

card(p1, 'i:write', BX, read.y + read.h + 55, BW, {
  pal: C.check,
  icon: 'shape=hexagon',
  iconText: 'запись',
  title: 'Запись — только по нажатию',
  lines: [
    'Дефект, комментарий, отчёт страницей. Удаления и слияния нет вовсе: всё, что панель умеет наружу, — создать и прокомментировать.',
  ],
});

const ext = card(p1, 'i:ext', CX, 150, CW, {
  pal: C.group,
  title: 'Внешние системы',
  lines: [
    'Jira и Confluence, GitHub или GitLab по токену, тест-менеджмент, отчёты CI. Каждая включается своей карточкой и своим ключом.',
  ],
});

const out = card(p1, 'i:out', CX, ext.y + ext.h + 55, CW, {
  pal: C.note,
  title: 'Уведомления — в обратную сторону',
  lines: [
    'Вебхук: один POST с JSON на ваш адрес, с подписью X-AgentDeck-Signature, если задан секрет. Telegram: сообщение боту в чат.',
    'Уходит только заголовок события: вид, текст и имя папки проекта — ни промпта, ни ответа агента, ни путей.',
  ],
});

card(p1, 'i:not', CX, out.y + out.h + 55, CW, {
  pal: C.warn,
  title: 'Чего не бывает',
  lines: [
    'Мёртвая интеграция не роняет ни прогон, ни разговор, ни старт панели: отказ виден состоянием карточки.',
    'Входа по SSO или паролю нет — только токен.',
  ],
});

link(p1, 'i:human', 'i:server', 'кнопка', { colour: FLOW.human, exit: [1, 0.3], entry: [0, 0.3] });
link(p1, 'i:agent', 'i:server', 'через MCP-прокси, без ключа', {
  colour: FLOW.fwd,
  exit: [1, 0.4],
  entry: [0, 0.7],
});
link(p1, 'i:token', 'i:server', 'заголовок запроса', {
  colour: FLOW.data,
  exit: [1, 0.3],
  entry: [0, 0.9],
});
link(p1, 'i:server', 'i:read', 'что забрать', {
  colour: FLOW.fwd,
  exit: [0.5, 1],
  entry: [0.5, 0],
});
link(p1, 'i:read', 'i:write', 'и что отдать', {
  colour: FLOW.fwd,
  exit: [0.5, 1],
  entry: [0.5, 0],
});
link(p1, 'i:read', 'i:ext', 'запрос наружу', {
  colour: FLOW.fwd,
  exit: [1, 0.3],
  entry: [0, 0.6],
});
link(p1, 'i:write', 'i:out', 'события прогона', {
  colour: FLOW.data,
  exit: [1, 0.4],
  entry: [0, 0.5],
});
...
legend(p1, 60, bottomOf(p1) + 60, [
  { caption: 'человек и его нажатие', fill: C.human.tint, stroke: C.human.line },
  { caption: 'агент и его CLI', fill: C.cli.tint, stroke: C.cli.line },
  ...
  { caption: 'внешние системы', fill: C.group.tint, stroke: C.group.line },
  { caption: 'что уходит наружу', fill: C.note.tint, stroke: C.note.line },
