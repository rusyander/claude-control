...
/**
 * След ПОВТОРНОГО применения: новые хеши, но копии и каталоги — от первого.
 *
 * Второе применение в ту же цель снимает копии с файлов, которые перенос уже
 * переписал, и положи панель такой след поверх прежнего — «отменить» вернуло бы
 * файлы к состоянию ПОСЛЕ первого переноса, то есть не вернуло бы ничего, и
 * отчиталось бы числом отменённых файлов. Того состояния, ради которого кнопка
 * и существует, в памяти панели не осталось бы вовсе.
 *
 * Поэтому от прежнего следа берётся то, что описывает мир ДО панели, — копия и
 * созданный ею каталог, — а от нового только отпечаток записанного: по нему
 * отмена узнаёт руку человека, и он обязан быть свежим. Файл, которого в
 * прежнем следе не было, едет как есть: его перенос трогает впервые.
 *
 * Файлы, которые второе применение НЕ тронуло, из следа не выпадают: первое их
 * изменило, изменёнными они и лежат, и забыть их значило бы оставить их без
 * отмены навсегда. Идут они первыми — в том порядке, в каком их писали, потому
 * что отмена возвращает файлы с конца.
 */
export function keepFirstBackups(
  previous: TransferRecord | undefined,
  files: readonly TransferFileRecord[],
): TransferFileRecord[] {
  if (!previous) return [...files];
  const before = new Map(previous.files.map((file) => [file.filePath, file]));
  const now = new Set(files.map((file) => file.filePath));
  const untouched = previous.files.filter((file) => !now.has(file.filePath));
  return [
    ...untouched,
    ...files.map((file) => {
      const first = before.get(file.filePath);
      return first ? { ...file, backupPath: first.backupPath, createdDir: first.createdDir } : file;
    }),
  ];
}
