...
| What                                                           | Why                                                                                                                                                                                                   |
| -------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The phone does not edit configuration — chat, files, analytics | **by design.** A phone screen is no place for a rules and hooks editor; the scope was the user's own choice                                                                                           |
| Project files on the phone are read-only                       | **by design.** The agent edits, the person watches; writing stays in the panel                                                                                                                        |
| Push notifications do not work                                 | **foreign.** Expo Push needs `eas init` under your own account and a Firebase/FCM project. Until then the app says so, and a local notification still fires                                           |
| Reachable only over your own Tailscale network                 | **by design.** The server stays on `127.0.0.1` and the tunnel terminates on your machine. Opening a port outward is not an option: the token is the only barrier and it is meant for your own network |
| Panel off — phone dead                                         | **by design.** That machine is what runs the agents; there is no cloud, and that is decided rather than forgotten                                                                                     |
