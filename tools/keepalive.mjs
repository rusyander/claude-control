 ...
 * осталась без надзора: dev-сторож (`dev-watch.mjs`) уже не поднимет упавший API, а vite без
 ...
 * %LOCALAPPDATA%\agentdeck\ заставляет лишний экземпляр молча выйти.
 */
...
const UNITS = [
  { id: 'server', port: Number(process.env.PORT ?? 5178), filter: '@agentdeck/server' },
  { id: 'web', port: Number(process.env.WEB_PORT ?? 8888), filter: '@agentdeck/web' },
];

const PROBE_MS = 20_000;
/** Холодный старт vite и dev-сторожа API — секунды, но на занятой машине десятки. */
 ...
 * быть живо наполовину: уборщик процессов срубает `pnpm`/`cmd`, а dev-сторож API
