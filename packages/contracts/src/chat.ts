...
export const chatSummarySchema = object({
  ...
  /**
   * Глубина продумывания, назначенная разговору панелью (разбор, звено
   * разделения, ревью на потолке).
   *
   * Из связи: транскрипт глубину не пишет вовсе. Без неё шапка такого чата
   * показывала глубину из настроек, а не ту, на которой он идёт (живой прогон
   * 24.09: разбор на xhigh, в шапке «Высокая»), и следующее сообщение человека
   * уезжало на настройке, а не на назначенной глубине.
   */
  effort: string().optional(),
  /**
   * Модель, назначенная разговору панелью (разбор, группа разделения, звено
   * конвейера), — из связи, по тому же правилу, что и `effort`. Отдельно от
   * `model`: та — чем разговор ШЁЛ по транскрипту (у любого чата), а эта —
   * чем панель велела ему идти. Без неё следующее сообщение человека уезжало
   * на модели из настроек: панель шлёт свою модель, и назначенная проигрывала.
   */
  assignedModel: string().optional(),
  /**
  ...
  /**
   * Разговор дерева ждёт человека: вопрос текстом, форма вопроса или запрос
   * прав (итоговое ревью 25.09). Из записей панели, а не из транскрипта; без
   * метки в списке ждущий ребёнок неотличим от закончившего.
   */
  awaitsYou: boolean().optional(),
  /**
   * Группа разделения, чей это разговор, принята человеком кнопкой «Принять».
   * Только у чата группы, который открывает строка хаба, — не у каждого звена.
   */
   ...
   * (владелец, 24.09.2026: «группы разделения в работе — наверх»).
   */
  inWork: boolean().optional(),
  /**
   * Звено разделения, снятое перезапуском групп: прогон остановлен, группа
   * заведена заново другим чатом. Связь с родителем помнится (`parentId`), чтобы
   * разговор оставался в дереве, а не всплывал корнем, — но группой он больше не
   * является: номера группы и стадии у такого чата нет. Имя группы остаётся —
   * им снятое звено и называется, когда своих слов в нём нет.
   */
  retired: boolean().optional(),
  /**
   * Снятое звено оставило неубранную копию (F5.2): новое разделение того же
   * разговора заводит свои группы, а копия старой лежит на диске. Список даёт
   * убрать её кнопкой — хаб её уже не показывает, план у родителя новый.
   */
  copyLeft: boolean().optional(),
  /**
   * Своего названия у разговора нет: ни заголовка Claude Code, ни слов человека —
   * только преамбулы панели (задание звена ревью, правок, доставки). `title`
   * тогда — имя проекта-заглушка, и список заменяет его именем группы из связи.
   * То же, когда первая реплика — панели, а `title` взят из следующей: у звена
   * группы это ответ человека на её вопрос, а не название.
   */
  untitled: boolean().optional(),
});
...
/**
 * Авторежим прав чата (`GET /api/chat/:chatId/auto-mode`). `override` — выбор
 * человека в этом чате, сильнее глобальной настройки в обе стороны; нет его —
 * чат идёт за `global` (`AppSettings.chatAutoMode`). `enabled` — что действует.
 */
export interface ChatAutoModeView {
  enabled: boolean;
  override?: boolean;
  global: boolean;
}
