 ...
 *
 * Песочница пишет в ~/.agentdeck/sandboxes — тест использует уникальный
...
describe('SandboxConfig', () => {
  ...
  it('sandboxPaths кладёт песочницы вне настоящего каталога .claude', () => {
    ...
    expect(sandboxRoot).toContain('.agentdeck');
  });
  ...
  it('рабочая папка едет в sandbox.env, а не в глобальный process.env', () => {
    delete process.env.AGENTDECK_SANDBOX_WORKDIR;
    ...
    expect(sandbox.env.AGENTDECK_SANDBOX_WORKDIR).toBe(sandbox.workDir);
    ...
    expect(process.env.AGENTDECK_SANDBOX_WORKDIR).toBeUndefined();
  });
