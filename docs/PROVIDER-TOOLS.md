...
Rules live in `<config dir>/agentdeck/dlp-rules.json`, apart from `state.json`: their
...
`agentdeck-prompt-gate.mjs` script into the configuration hooks directory and registers it in
