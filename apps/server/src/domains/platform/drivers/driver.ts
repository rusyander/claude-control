 ...
 * других другая (401 у платформы компании означает пять разных вещей — у произвольного
...
export interface DriverControl {
  ...
  /**
   * Допустимые значения, когда их набор конечен (`platform_tool_mode`). Пусто —
...
export interface PlatformDriver {
   ...
   * api.<домен>» верно для платформы компании и неверно для произвольного шлюза, который
   ...
   * читает по именам `platform_*`) был написан первым и разбирался ровно до
  ...
  /**
   * Префикс вендорных полей, когда драйвер их читает (`<префикс>_status`).
   * `undefined` — вендорных полей у драйвера нет, и переопределение префикса
   ...
   * на экране нет вовсе (аудит DRV-12: форма платформы компании `agent/completions`
...
export function imageCapability(models: PlatformModelInfo[]): PlatformCapabilityFinding {
  ...
  if (!declared) {
    return {
      ...
      detailCode:
        models.length > 0 ? 'capability-image-flag-undeclared' : 'capability-image-no-models',
      evidence: 'answer',
  ...
  return {
    ...
    detailCode: drawing > 0 ? 'capability-image-listed' : 'capability-image-none',
    evidence: 'answer',
    compromise: 'media-by-capability',
