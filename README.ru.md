...
- **Запись защищена.** Резервная копия в `~/.claude/agentdeck/backups/`, затем атомарная запись через временный файл: прерванное сохранение не оставит недописанный `settings.json`.
...
| `~/.claude/agentdeck/`                                       | чтение / запись   | Состояние самой панели: группы, сценарии, настройки, резервные копии          |
| `~/.agentdeck/`                                              | чтение / запись   | Рабочие папки чатов и временные каталоги песочниц — намеренно вне `~/.claude` |
...
Введённые ключи лежат зашифрованными (AES-256-GCM) в `agentdeck/provider-keys.enc`, фраза — машинно-локальный файл с правами `0600`. Наружу отдаётся только маска `sk-…1234`. В логи, копии, историю, поиск и архив переноса окружения ключи не попадают — на это есть тест по всем провайдерам.
...
**Ключи не попадают в git.** Сухой прогон коммита, который сделал бы участник проекта: `git add -An --all` берёт только исходники, история чистая, а внутрь репозитория приложение не пишет вообще — все пути записи ведут в `~/.claude` или `~/.agentdeck`. Поэтому и вложения чата лежат в папке панели: `git add -A` их не заберёт.
