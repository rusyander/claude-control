 ...
 * что-то своё. Значения своих меток контур возвращает в поток сам; карту кадром
 * он присылает только собственному чату (платформа компании, `is_chat_caller`), и там, где
 * она есть, она разворачивается здесь. Для клиента API остаётся другое: метка
 * вида платформы, дошедшая до аргумента, — значение, которое никто не вернул.
 *
...
/**
 * Метки САМОЙ платформы в аргументах вызова, которые никто не развернул.
 *
 * Карту подмены платформа клиенту API не отдаёт — значения она возвращает в
 * поток сама, своим деанонимизатором. Значит, метка её вида, дошедшая до
 * аргумента, — это значение, которое вернуть не вышло: модель исказила номер или
 * метка разрезалась так, что платформа её не узнала. Исключение одно: ровно та же
 * строка была в теле, ушедшем наверх, — тогда это текст человека, а не подмена.
 * Решать, подменяла ли платформа вообще, — дело вызывающего.
 */
export function strayPlatformLabels(
  args: Record<string, unknown>,
  pattern: RegExp,
  sent: string,
): string[] {
  ...
  mapStrings(args, (text) => {
    for (const match of text.matchAll(global)) {
      const token = match[0];
      if (sent.includes(token) || stray.includes(token)) continue;
      stray.push(token);
    }
    return text;
  });
