 ...
 *    понижение поколения.
 *
 * Чего здесь НЕТ и почему: самого конвейера «работа → ревью → правки». Решает
 * про звенья `domains/provider-chat/cascade.ts` — он висит на завершении ответа
 * в `ProviderChatService`, а не на реестре прогонов Claude, которого чужие
 * разговоры не касаются вовсе. Здесь остаётся ровно подбор ступени; ревьюером в
 * том конвейере работает сам CLI своей настройкой — прогон без единого флага
 * модели, потому что другого «выше» у чужого провайдера не существует.
 */
