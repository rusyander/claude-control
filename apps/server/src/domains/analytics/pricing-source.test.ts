...
describe('PricingStore', () => {
  ...
  it('обновление сохраняет прайс на диск и делает его актуальным', async () => {
    // Страницу подменяем: без подмены тест ходил в живой prices-документ и краснел
    // от сетевой икоты — на настоящей странице он не проверяет ничего, чего не
    // проверяет разбор выше, а среда здесь заявлена без сети.
    stubOk();
    const store = new PricingStore(dir);
    await store.refresh();
