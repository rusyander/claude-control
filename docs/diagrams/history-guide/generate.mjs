/**
 * Схема путеводителя «История изменений». Источник — этот генератор, `.drawio` —
 * его вывод:
 *
 *   node docs/diagrams/history-guide/generate.mjs <абсолютный путь>.drawio
 *
 ...
 *
 * Страница одна. Лента на снимке выглядит как журнал действий, а на самом деле
 * это РАЗНИЦЫ между копиями файла: копий нет — нет и записи, а «против текущего
 * файла» — единственное место, где видно правку, сделанную мимо панели.
 */
...
// ─── Страница: откуда берётся лента и что делает возврат ───────────────────

const p1 = newPage('Откуда берётся лента', 'p1');
heading(
  p1,
  60,
  30,
  'Откуда берётся лента и что именно пишет возврат',
  'Журнала действий панель не ведёт. Лента — это разницы между копиями, которые снимаются перед каждой её записью.',
  1240,
);
...
const write = card(p1, 'h:write', AX, 150, AW, {
  pal: C.panel,
  icon: 'shape=card',
  iconText: 'правка',
  title: 'Любая запись панели',
  lines: [
    'Правило, хук, право, переменная, MCP-сервер, CLAUDE.md — перед записью снимается копия текущего файла, и только потом файл переписывается.',
  ],
});

const keep = card(p1, 'h:keep', AX, write.y + write.h + 55, AW, {
  pal: C.store,
  icon: 'shape=note;size=14',
  iconText: 'bak',
  title: 'Каталог копий',
  lines: [
    'agentdeck/backups, имя вида <файл>.<метка времени>.bak. Время берётся из ИМЕНИ, а не из даты файла.',
    'Хранится десять копий каждого файла (в настройках 1..100); лишние удаляются с самых старых. Отмотать дальше последней копии нельзя.',
  ],
});

card(p1, 'h:hand', AX, keep.y + keep.h + 55, AW, {
  pal: C.human,
  icon: 'shape=card',
  iconText: 'вы',
  title: 'Правка руками в редакторе',
  lines: [
    'Копии не делает никто. Такая правка появится в ленте единственным способом: как разница самой свежей копии с текущим файлом.',
  ],
});

const feed = card(p1, 'h:feed', BX, 150, BW, {
  pal: C.check,
  icon: 'shape=hexagon',
  iconText: 'лента',
  title: 'Как складывается лента',
  lines: [
    'Копии группируются по файлу и выстраиваются по времени. Каждая сравнивается с ПРЕДЫДУЩЕЙ — это и есть правка, которая к ней привела.',
    'Самая свежая копия сравнивается с ТЕКУЩИМ файлом на диске: строка «против текущего файла» показывает ещё не сохранённую в копию правку.',
    'У самой старой копии предыдущей нет — она помечена «первая известная версия», диффа против неё не строится.',
  ],
});

const diff = card(p1, 'h:diff', BX, feed.y + feed.h + 55, BW, {
  pal: C.check,
  icon: 'shape=hexagon',
  iconText: '±',
  title: 'Дифф и его границы',
  lines: [
    'Построчный, направление хронологическое: «+N / −M» читается как «столько добавили и убрали этой правкой».',
    'Двоичный файл, больше 512 КБ или 5000 строк — дифф не строится: лента остаётся дешёвой, а не показывает мусор.',
  ],
});

card(p1, 'h:foreign', BX, diff.y + diff.h + 55, BW, {
  pal: C.cli,
  icon: 'shape=process',
  iconText: 'CLI',
  title: 'Копии чужого CLI — только просмотр',
  lines: [
    'Файлы активного провайдера попадают в ленту и показывают дифф, но возврата у них нет: цель копии выводится по имени, и ошибка означала бы запись чужого конфига поверх файлов Claude.',
  ],
});

const list = card(p1, 'h:list', CX, 150, CW, {
  pal: C.panel,
  title: 'Что видно в ленте',
  lines: [
    'Файл, время, против чего считался дифф и ±строк. Запись раскрывается в построчный дифф прямо на месте.',
  ],
});

const revert = card(p1, 'h:revert', CX, list.y + list.h + 55, CW, {
  pal: C.panel,
  title: 'Возврат одного блока',
  lines: [
    'Дифф пересчитывается заново как «копия → текущий файл», поэтому возвращается ровно тот блок, который показан.',
    'Пишется ТЕКУЩИЙ файл, а не копия поверх всего. Перед записью снимается ещё одна копия — сам возврат тоже обратим.',
  ],
});

card(p1, 'h:not', CX, revert.y + revert.h + 55, CW, {
  pal: C.warn,
  title: 'Чего в истории нет',
  lines: [
    'Кто именно правил: панель не различает человека и агента, у файла нет автора.',
    'Правок ваших проектов и разговоров — это git и чат. Отката файла целиком: он живёт в настройках, рядом со списком копий.',
  ],
});

link(p1, 'h:write', 'h:keep', 'копия перед записью', {
  colour: FLOW.fwd,
  exit: [0.5, 1],
  entry: [0.5, 0],
});
link(p1, 'h:hand', 'h:feed', 'видна только как разница с текущим файлом', {
  colour: FLOW.human,
  exit: [1, 0.4],
  entry: [0, 0.9],
});
link(p1, 'h:keep', 'h:feed', 'все копии файла', {
  colour: FLOW.data,
  exit: [1, 0.3],
  entry: [0, 0.6],
});
link(p1, 'h:feed', 'h:diff', 'пара версий', {
  colour: FLOW.fwd,
  exit: [0.5, 1],
  entry: [0.5, 0],
});
link(p1, 'h:diff', 'h:list', 'строки и ханки', {
  colour: FLOW.fwd,
  exit: [1, 0.3],
  entry: [0, 0.7],
});
link(p1, 'h:feed', 'h:list', 'записи', { colour: FLOW.fwd, exit: [1, 0.3], entry: [0, 0.3] });
link(p1, 'h:list', 'h:revert', 'кнопка у блока', {
  colour: FLOW.fwd,
  exit: [0.5, 1],
  entry: [0.5, 0],
});
