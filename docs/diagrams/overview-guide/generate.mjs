/**
 * Схема путеводителя «Обзор». Источник — этот генератор, `.drawio` — его вывод:
 *
 *   node docs/diagrams/overview-guide/generate.mjs <абсолютный путь>.drawio
 *
 ...
 *
 * Страница одна, и этого достаточно. Весь раздел отвечает на один вопрос:
 * откуда взялось число на плитке. На экране видно само число, но не видно, что
 * своей базы у панели нет вовсе и каждое число — счёт по файлам каталога,
 * сделанный в момент открытия страницы. Остальное показывают снимки.
 */
...
// ─── Страница: откуда берётся каждое число обзора ──────────────────────────

const p1 = newPage('Откуда берутся числа обзора', 'p1');
heading(
  p1,
  60,
  30,
  'Откуда берётся каждое число обзора',
  'Своей базы у панели нет. Плитка — это счёт по файлам каталога конфигурации, сделанный в момент открытия страницы.',
  1200,
);
...
const dir = card(p1, 'o:dir', AX, 150, AW, {
  pal: C.panel,
  icon: 'shape=card',
  iconText: 'путь',
  title: 'Какой каталог читается',
  lines: [
    'Три правила подряд, побеждает первое совпавшее: путь, заданный руками в настройках → переменная CLAUDE_CONFIG_DIR → ~/.claude.',
    'Бейдж на карточке называет сработавшее правило. Заданный руками путь сильнее переменной — и остаётся заданным, пока его не сменят.',
  ],
});

const files = card(p1, 'o:files', AX, dir.y + dir.h + 55, AW, {
  pal: C.store,
  icon: 'shape=note;size=14',
  iconText: 'файлы',
  title: 'Файлы этого каталога',
  lines: [
    'CLAUDE.md, settings.json и settings.local.json, .claude.json, папки skills/ и hooks/.',
    'Они и есть источник правды: панель ничего не дублирует у себя, поэтому правка через редактор и правка через панель для неё одинаковы.',
  ],
});

card(p1, 'o:own', AX, files.y + files.h + 55, AW, {
  pal: C.store,
  icon: 'shape=note;size=14',
  iconText: 'своё',
  title: 'Два файла самой панели',
  lines: [
    'state.json — группы, включения-выключения и результат ПОСЛЕДНЕЙ проверки связи с MCP-серверами.',
    'agentdeck/backups — копии, которые панель снимает перед каждой своей записью.',
  ],
});

const count = card(p1, 'o:count', BX, 150, BW, {
  pal: C.check,
  icon: 'shape=hexagon',
  iconText: 'счёт',
  title: 'Обход при открытии страницы',
  lines: [
    'Считают те же читалки, что и сами разделы, — иначе обзор и раздел разошлись бы в цифрах.',
    'Попутно проверяется то, что видно только на диске: хук, чей скрипт не существует, — «сломан»; скрипт, на который не ссылается ни один хук, — «не привязано» (тесты и фикстуры не в счёт).',
    'Состояние MCP-сервера НЕ проверяется при открытии: берётся результат последней проверки связи.',
  ],
});

const feed = card(p1, 'o:feed', BX, count.y + count.h + 55, BW, {
  pal: C.check,
  icon: 'shape=hexagon',
  iconText: '7 дн.',
  title: 'Сводка изменений',
  lines: [
    'Та же лента, что на странице истории: записи, собранные по копиям, с отбором за последние семь дней.',
    'Одна правка одного файла — одна запись. Правка в редакторе мимо панели попадёт сюда только как разница последней копии с текущим файлом.',
  ],
});

card(p1, 'o:watch', BX, feed.y + feed.h + 55, BW, {
  pal: C.check,
  icon: 'shape=hexagon',
  iconText: 'live',
  title: 'Пересчёт без F5',
  lines: [
    'За каталогом следит наблюдатель: файл изменился — панель пересчитывает числа сама.',
    'Поэтому цифра, не сходящаяся с тем, что вы помните, почти никогда не «застряла»; смотреть надо на каталог сверху.',
  ],
});

const tiles = card(p1, 'o:tiles', CX, 150, CW, {
  pal: C.panel,
  title: 'Плитка: число, оговорка, ссылка',
  lines: [
    'Сверху — всего, снизу — уточнение: сколько включено, сколько сломано, сколько не привязано.',
    'Подпись краснеет ровно в двух случаях: есть хук со сломанным путём или есть MCP-сервер, который не ответил на последнюю проверку.',
    'Сама плитка — ссылка в раздел, где лежит ровно этот список; мелкие ссылки под ней открывают форму создания.',
  ],
});

const changes = card(p1, 'o:changes', CX, tiles.y + tiles.h + 55, CW, {
  pal: C.panel,
  title: 'Строка «Изменения за 7 дней»',
  lines: [
    'Число правок за период и переход в историю. Плитка «Резервные копии» рядом — сколько копий лежит на диске и когда снята последняя.',
  ],
});

card(p1, 'o:not', CX, changes.y + changes.h + 55, CW, {
  pal: C.warn,
  title: 'Чего на обзоре нет',
  lines: [
    'Расхода токенов и работающих агентов — это аналитика. Разговоров — это чат.',
    'Конфигурации ПРОЕКТА: обзор считает только глобальный каталог. Настроек чужого CLI: у каждого свои файлы, их показывают его собственные разделы.',
  ],
});

link(p1, 'o:dir', 'o:files', 'первое совпавшее правило', {
  colour: FLOW.fwd,
  exit: [0.5, 1],
  entry: [0.5, 0],
});
link(p1, 'o:files', 'o:count', 'читают читалки разделов', {
  colour: FLOW.data,
  exit: [1, 0.3],
  entry: [0, 0.4],
});
link(p1, 'o:own', 'o:count', 'группы и прошлая проверка связи', {
  colour: FLOW.data,
  exit: [1, 0.3],
  entry: [0, 0.9],
});
link(p1, 'o:own', 'o:feed', 'копии', { colour: FLOW.data, exit: [1, 0.7], entry: [0, 0.8] });
link(p1, 'o:files', 'o:watch', 'файл изменился', {
  colour: FLOW.fwd,
  dashed: true,
  exit: [1, 0.8],
  entry: [0, 0.2],
});
link(p1, 'o:count', 'o:tiles', 'число и его оговорка', {
  colour: FLOW.fwd,
  exit: [1, 0.4],
  entry: [0, 0.4],
});
link(p1, 'o:feed', 'o:changes', 'записи за семь дней', {
  colour: FLOW.fwd,
  exit: [1, 0.4],
  entry: [0, 0.5],
});
link(p1, 'o:watch', 'o:tiles', 'пересчёт сам', {
  colour: FLOW.fwd,
  dashed: true,
  exit: [1, 0.2],
  entry: [0, 0.9],
