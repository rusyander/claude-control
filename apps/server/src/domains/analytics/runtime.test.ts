...
import type { RunningAgent } from '@agentdeck/contracts';
import {
 ...
 * легко подводится лишнее — сама панель (её каталог зовётся agentdeck),
...
describe('isClaudeAgentCommand — кого считаем работающим агентом', () => {
  ...
  it('сама панель агентом не считается: её каталог agentdeck', () => {
  ...
  it('сервер панели по полному пути тоже не считается', () => {
    const command =
      '"C:\\Program Files\\nodejs\\node.exe" c:\\work\\agentdeck\\apps\\server\\src\\index.ts';
    expect(isClaudeAgentCommand(command)).toBe(false);
  });
