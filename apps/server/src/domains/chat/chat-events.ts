...
export type ChatEvent =
   ...
   * запрос прав (`toolUseId` — его), показанный другой карточкой: ответ на неё
   * разрешает или отклоняет ровно этот вызов.
   */
  | {
      kind: 'branchGate';
      toolName: string;
      input: unknown;
      toolUseId: string;
      /** Каталог, в котором идёт прогон, — он же основная копия репозитория. */
      cwd: string;
