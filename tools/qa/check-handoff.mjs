...
const block = (json) => ['```agentdeck:handoff', JSON.stringify(json), '```'].join('\n');
...
await page.route('**/api/chat/handoff/request', (route) =>
  route.fulfill({ json: { prompt: 'Закрой этап и выведи блок agentdeck:handoff.' } }),
);
...
check(!body.includes('agentdeck:handoff'), 'сырого блока в ленте нет');
...
await page.waitForTimeout(1500);
check(
  sent?.prompt === 'Закрой этап и выведи блок agentdeck:handoff.',
