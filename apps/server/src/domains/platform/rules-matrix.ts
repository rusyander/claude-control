 ...
 * инструментов остался). Отправить `enterprise-platform_tools` туда, где о нём не
...
export function ruleConflicts(
  ...
  // своё, и литерал платформа компании подсвечивал бы чужому контуру несуществующую строку.
  ...
  // первой, метки у неё другого вида, чем у контура, и его восстановление их не
  // трогает; свои метки контур возвращает значениями сам, прямо в потоке, а
  // карту клиенту API не отдаёт (router.py:596, `is_chat_caller`). Строка нужна
  // ровно затем, чтобы человек не выключил одно из двух в поисках
  ...
  if (declared(driver, 'anonymization')) {
    rows.push({
      ...
      title: 'Подмена данных контура ⟷ наша защита данных',
      detail:
        'Не выбор из двух: слои складываются по порядку. Наша маска накладывается первой и ' +
        'обратима, а свои метки контур сам возвращает значениями прямо в потоке ответа — ' +
        'карту подмены клиенту API он не отдаёт. Выключать ничего не нужно.',
