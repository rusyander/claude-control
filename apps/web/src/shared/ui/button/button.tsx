...
export function Button(props: ButtonProps) {
  ...
  return (
    <button
      ...
      // запрос ещё раз, а мы правим конфиги — дубли тут дорого стоят. Именно
      // «ИЛИ», а не «??»: с `??` любое явное `disabled` (в том числе `false` —
      // а его присылает каждая кнопка с условием вида `disabled={!hasToken}`)
      // отменяло бы это правило целиком, и кнопка оставалась бы нажимаемой все
      // те секунды, пока идёт запрос.
