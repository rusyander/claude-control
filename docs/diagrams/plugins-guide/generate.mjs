/**
 * Схема путеводителя «Плагины». Источник — этот генератор, `.drawio` — его вывод:
 *
 *   node docs/diagrams/plugins-guide/generate.mjs <абсолютный путь>.drawio
 *
 ...
 *
 * Страница одна: путь установки целиком. Это единственный раздел, где между
 * кнопкой и диском стоит посредник — сам CLI, — и почти все странности раздела
 * растут отсюда: без установленного claude не работает ничего, плагин не
 * оживает до следующей сессии, а запись в реестре переживает исчезновение
 * каталога. Словами это три разных абзаца, рисунком — одна цепочка.
 */
...
// ─── Страница: путь установки от кнопки до следующей сессии ────────────────

const p1 = newPage('Путь установки', 'p1');
heading(
  p1,
  60,
  30,
  'Что происходит при установке',
  'Между кнопкой панели и диском стоит сам CLI. Отсюда и требование установленного claude, и задержка до следующей сессии, и запись, пережившая свой каталог.',
  1200,
);
...
const panel = card(p1, 'g:panel', AX, 150, AW, {
  pal: C.panel,
  icon: 'shape=card',
  iconText: 'UI',
  title: 'Кнопка в разделе',
  lines: [
    'Установить, обновить, включить, удалить — любое действие только запускает команду и показывает её вывод целиком.',
    'Файлы плагина панель не правит: они принадлежат автору, и обновление перезапишет любую правку.',
  ],
});

const nocli = card(p1, 'g:nocli', AX, panel.y + panel.h + 55, AW, {
  pal: C.warn,
  icon: 'shape=hexagon',
  iconText: '!',
  title: 'Нет claude в PATH — не работает ничего',
  lines: [
    'Раздел говорит об этом строкой. Соседние разделы в той же ситуации работают как обычно: они читают файлы, а этот — вызывает программу.',
  ],
});

card(p1, 'g:sources', AX, nocli.y + nocli.h + 55, AW, {
  pal: C.store,
  icon: 'shape=note;size=14',
  iconText: 'json',
  title: 'Источники — маркетплейсы',
  lines: [
    'Список лежит в plugins/known_marketplaces.json; новый добавляют командой marketplace add по ссылке или пути.',
    'Выключенный маркетплейс скрывает из каталога все свои плагины разом, уже установленные при этом остаются.',
  ],
});

const cli = card(p1, 'g:cli', BX, 150, BW, {
  pal: C.cli,
  icon: 'shape=process',
  iconText: 'CLI',
  title: 'Команда claude plugin …',
  lines: [
    'Та же, что вы набрали бы в терминале. Ошибку объясняет она, а не панель, — это единственный источник правды о том, что не вышло.',
    'Ожидание разное: список установленных — до минуты, каталог доступных — до трёх, он ходит в сеть.',
  ],
});

const market = card(p1, 'g:market', BX, cli.y + cli.h + 55, BW, {
  pal: C.check,
  icon: 'shape=process',
  iconText: 'www',
  title: 'Маркетплейс',
  lines: [
    'Обычно репозиторий на GitHub. Без сети не работают ни каталог, ни установка, ни обновление.',
  ],
});

card(p1, 'g:scaffold', BX, market.y + market.h + 55, BW, {
  pal: C.panel,
  icon: 'shape=folder',
  iconText: 'new',
  title: 'Каркас своего плагина — единственная запись панели',
  lines: [
    'Четыре файла в выбранную вами папку, а не в каталог настроек.',
    'Нигде не регистрируется: чтобы такой плагин заработал, его публикуют в маркетплейс и ставят оттуда обычным путём.',
  ],
});

const disk = card(p1, 'g:disk', CX, 150, CW, {
  pal: C.store,
  icon: 'shape=folder',
  iconText: 'dir',
  title: 'Каталог плагина на диске',
  lines: [
    'Скиллы, хуки, команды и MCP-серверы автора. Видны в своих разделах панели, но помечены как чужие.',
  ],
});

const registry = card(p1, 'g:registry', CX, disk.y + disk.h + 55, CW, {
  pal: C.store,
  icon: 'shape=note;size=14',
  iconText: 'json',
  title: 'Запись в реестре, версия 2',
  lines: [
    'Включённым считается тот, у кого в installed_plugins.json явно стоит «да»: выключение меняет запись, каталог остаётся на месте.',
    'Панель читает только версию 2 — ровно ту, что читает CLI; другая версия ⇒ плагинов не видно нигде, и причина названа строкой.',
  ],
});

const session = card(p1, 'g:session', CX, registry.y + registry.h + 55, CW, {
  pal: C.group,
  icon: 'shape=process',
  iconText: '→',
  title: 'Подхватится следующей сессией',
  lines: [
    'Каталоги CLI читает при старте. Открытый разговор нового плагина не увидит — это не ошибка установки.',
  ],
});

card(p1, 'g:missing', CX, session.y + session.h + 55, CW, {
  pal: C.warn,
  icon: 'shape=card',
  iconText: '?',
  title: 'Пометка «каталог не найден»',
  lines: [
    'Каталог убрали мимо панели, а запись в реестре осталась: CLI по-прежнему считает плагин установленным.',
    'Карточка говорит об этом прямо — иначе плагин выглядел бы рабочим и молча ничего не делал.',
  ],
});

link(p1, 'g:panel', 'g:cli', 'запускает команду', {
  colour: FLOW.human,
  exit: [1, 0.4],
  entry: [0, 0.4],
});
link(p1, 'g:nocli', 'g:cli', 'запускать нечего', {
  colour: FLOW.back,
  dashed: true,
  exit: [1, 0.3],
  entry: [0, 0.9],
});
link(p1, 'g:sources', 'g:market', 'откуда брать', {
  colour: FLOW.data,
  exit: [1, 0.3],
  entry: [0, 0.85],
});
link(p1, 'g:cli', 'g:market', 'идёт в источник', {
  colour: FLOW.fwd,
  exit: [0.5, 1],
  entry: [0.5, 0],
});
link(p1, 'g:market', 'g:disk', 'кладёт каталог', {
  colour: FLOW.fwd,
  exit: [1, 0.3],
  entry: [0, 0.8],
});
// Правая колонка — одна вертикальная цепочка: каталог, запись о нём, старт
// сессии. Стрелка от команды сразу к реестру шла бы поверх карточки каталога, а
// порядок «сначала положили, потом записали» от этого только теряется.
link(p1, 'g:disk', 'g:registry', 'и запись о нём', {
  colour: FLOW.fwd,
  exit: [0.5, 1],
  entry: [0.5, 0],
});
link(p1, 'g:registry', 'g:session', 'подхватится при старте', {
  colour: FLOW.fwd,
  exit: [0.5, 1],
  entry: [0.5, 0],
});
link(p1, 'g:registry', 'g:missing', 'запись пережила каталог', {
  colour: FLOW.back,
  dashed: true,
  exit: [1, 0.5],
  entry: [1, 0.5],
  points: [{ x: CX + CW + 60, y: registry.cy }],
});
link(p1, 'g:scaffold', 'g:registry', 'нигде не регистрируется', {
  colour: FLOW.back,
  dashed: true,
  exit: [1, 0.2],
  entry: [0, 0.95],
...
legend(p1, 60, bottomOf(p1) + 60, [
  ...
  { caption: 'сеть и источник', fill: C.check.tint, stroke: C.check.line },
  { caption: 'файл на диске', fill: C.store.tint, stroke: C.store.line },
  { caption: 'когда заработает', fill: C.group.tint, stroke: C.group.line },
  { caption: 'повод проверить', fill: C.warn.tint, stroke: C.warn.line },
  { caption: 'действие человека', edgeColour: FLOW.human },
  { caption: 'что делает CLI', edgeColour: FLOW.fwd },
