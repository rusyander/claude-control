...
const SCENARIOS = [
  ...
  },
  {
    ...
    note:
      'ТАБЛИЦА ОШИБАЛАСЬ: «отдельным текстом» невозможно, и причин ПЯТЬ. Проверка ключа отвечает ' +
      '«key expired» и «budget exceeded», проверка владельца добавляет «invalid API key», «key owner is ' +
      'deleted» и «key owner check failed»; на границе публичного API причина теряется — ' +
    ...
    closedBy: [
      ...
      // (волна C: драйвер зовёт `serverText('gateway-key-rejected')`, русская сторона
      // живёт в таблице). Якорь идёт за ТЕКСТОМ, а не за файлом: он стережёт
      // формулировку, а не её адрес, — поэтому переехал вместе с ней.
      [`${SERVER}/lib/server-texts/templates.ts`, 'истёк по сроку'],
    ],
  },
  {
    id: '7',
    ...
    closedBy: [
      ...
      // Текст живёт в таблице кодов (волна C), драйвер зовёт его по коду.
      [`${SERVER}/lib/server-texts/templates.ts`, 'исчерпал свой бюджет'],
    ],
  },
  {
    id: '7б',
    ...
    note:
      'ТАБЛИЦА ОШИБАЛАСЬ: на /v1 402 — только бюджет КЛЮЧА (обе публичные ветки, чата и агента), ' +
      'трёхуровневый бюджет над ключом проверяется лишь на JWT-маршрутах, ' +
