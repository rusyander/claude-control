 ...
 * обратно на модели работы. `fix` конечен: ревью второго круга не бывает, иначе
 * пара «проверил — поправил» крутилась бы, пока не упрётся в потолок цепочки.
 *
 ...
 */
export const CASCADE_STAGES = ['triage', 'plan', 'work', 'review', 'fix', 'push'] as const;
...
export function fixStagePrompt(
  ...
  const { branch, reviewer = 'ceiling' } = options;
...
export function reviewLinkPrompt(input: {
  ...
  const { url, branch, onMrBranch = true, tasks = [], shared } = input;
  ...
  return [
    ...
    'НИЧЕГО НЕ ПРАВЬ и ничего не пиши в MR: решение, чинить или отписать, принимает человек ' +
      'по карточке панели.',
    tasks.length > 0 ? 'На что просили посмотреть особо:' : '',
...
export function mergeRequestWorkPreamble(input: {
  ...
  const { url, branch, onMrBranch = true } = input;
  return [
 ...
 */
export function reviewPushPrompt(input: { url: string; branch?: string }): string {
  const where = input.branch ? `ветку ${input.branch}` : 'ветку этой копии';

  return [
    `Человек нажал «Закоммитить и отправить в MR» — это явное согласие на коммит и push в ${where}.`,
    `Закоммить сделанные по ревью правки одним осмысленным коммитом и отправь их в ${where}, ` +
      `откуда их подхватит ${input.url}.`,
