...
- **MCP-серверы** — с настоящей проверкой связи по протоколу MCP: stdio, HTTP и SSE; интерактивный вход OAuth для сетевых
- **Права** — allow / ask / deny, включая шаблоны `mcp__*`
- **Переменные** — из `settings.json` и `.mcp-secrets.env`, секреты замаскированы
- **Плагины** — установленные и каталог маркетплейсов, с добавлением и удалением источников
...
- **Чат** — разговор с Claude Code прямо в панели: стриминг, вложения, голос, предпросмотр артефактов, выбор модели и глубины, ветвление правкой сообщения, кликабельные варианты ответа
- **Несколько проектов сразу** — вкладки проектов, агенты работают параллельно и не гаснут при переключении
- **Аналитика** — расход токенов и оценочная стоимость по локальным транскриптам, с выгрузкой в CSV и JSON
...
flowchart LR
    subgraph browser["Браузер · localhost:8888"]
        UI["Интерфейс React<br/>15 разделов"]
    end
