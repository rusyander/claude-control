...
describe('agentRuns — очередь дописанного', () => {
  ...
  /**
   * Регрессия: досылка нарвалась на «прогон занят». В разговоре успел начаться
   * новый ход (телефон, соседнее окно, либо сервер ещё не снял прошлый), и
   * дописанное исчезало совсем — из очереди вынуто, до агента не доехало. Для
   * ответа на вопрос агента это худший случай: человек выбрал вариант, увидел
   * «в очереди», и больше о нём никто не вспомнил.
   */
  it('отказ «прогон занят» возвращает дописанное в очередь, а не съедает', async () => {
    const busy = {
      ok: false,
      status: 409,
      json: async () => ({ code: 'run_busy', message: 'занят', runId: 'q-5' }),
    } as unknown as Response;

    // Первый ход завершается сразу, а досылку встречает отказ.
    fetchMock.mockImplementationOnce(async () => sseResponse([DONE]));
    fetchMock.mockImplementation(async () => busy);

    void agentRuns.start({ chatId: 'q-5', prompt: 'первое' });
    agentRuns.enqueue('q-5', { prompt: 'выбранный вариант' });
    await settle();

    expect(getRun('q-5').queued.map((item) => item.prompt)).toEqual(['выбранный вариант']);
  });
