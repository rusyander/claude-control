...
🇬🇧 [English version](README.md) · 🔧 [Установка](docs/SETUP.ru.md) · 🛠 [Починка](docs/TROUBLESHOOTING.ru.md) · 🏗 [Как устроено](docs/ARCHITECTURE.ru.md) · 🔒 [Безопасность](docs/SECURITY.ru.md) · 📱 [Доступ с телефона](docs/REMOTE.ru.md) · 💬 [Чат и параллельные агенты](docs/CHAT.ru.md) · 👥 [Группы](docs/GROUPS.ru.md) · 📡 [Контур](docs/PLATFORM.ru.md) · 🧑‍💻 [Разработка](docs/DEVELOPMENT.ru.md) · 🚫 [Чего панель не делает](docs/LIMITATIONS.ru.md)

> [!TIP]
> **Не запускается?** `pnpm doctor` объяснит каждую находку. Не помогло — откройте в этой папке
> Claude Code и скажите «не запускается, разберись»: он прочитает [AGENTS.md](AGENTS.md), карту
...
- **Продолжение в чистой сессии** — закрыв задачу, агент прибирает свои рабочие файлы и предлагает продолжить в новом разговоре: дорогой контекст остаётся позади, а новая сессия читает файл-опору. По кнопке — или само: автопродолжение включено из коробки и держится предохранителями (упавший прогон не продолжают, не изменившийся файл-опора останавливает цепочку, подряд не больше восьми раз). Работает и у других CLI: сессии у них нет, поэтому продолжение — новый разговор с файлом-опорой и исходной задачей
...
- **Разбивка списка задач по чатам** и **продолжение в чистой сессии** — разбивку агент предлагает карточкой, решение ваше; продолжение панель делает и сама, а выключатель — тумблер разговора или общая настройка.
...
Что именно работает у каждого CLI — таблица «раздел × провайдер» в [Чего панель не делает у других CLI](docs/LIMITATIONS-PROVIDERS.ru.md#карта-раздел--провайдер). Пути к файлам, правящиеся ключи и причина каждого статуса — в [Провайдеры: детали форматов](docs/PROVIDERS.ru.md). Как перенести среду целиком из одного CLI в другой и что при этом доедет в каком виде — в [Перенос среды между CLI](docs/PORTABILITY.ru.md).
...
Можно свободно использовать, изменять и распространять: дома и на работе, внутри компании и в
коммерческих проектах. Единственное существенное условие: если вы изменили панель и даёте другим
пользоваться своей версией по сети, вы обязаны предложить им исходники этой версии под той же
лицензией (раздел 13). Гарантий нет.

Ранние снимки выходили под MIT, а с 05.08.2026 под PolyForm Perimeter 1.0.1: на те коммиты
разрешения сохраняются. Использование на других условиях обсуждается с автором отдельно.
