 ...
 *
 * Встроенные образцы — не здесь, а в `../dlp/builtins.mjs`, одной копией с
 * прокси. Импорт ниже при сборке хука снимается: скрипт получает текст образцов
 * вставкой прямо перед ядром (`script.ts`).
 */
...
function matchesOf(text, rule) {
  ...
  const builtin = rule.kind === 'builtin' ? DLP_BUILTINS[rule.builtin] : undefined;
