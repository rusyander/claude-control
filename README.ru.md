...
- В песочницах внутри `~/.agentdeck/sandboxes/` лежит копия `.credentials.json` и значения `env` MCP-серверов открытым текстом. Они удаляются при закрытии песочницы, но после аварийного завершения могут остаться — удалите папку, если хотите быть уверены.
