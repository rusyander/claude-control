...
- Sandboxes under `~/.agentdeck/sandboxes/` contain a copy of `.credentials.json` and any MCP server `env` values in plain text. They are removed when you close the sandbox, but a crash can leave one behind; delete the folder if you want to be sure.
