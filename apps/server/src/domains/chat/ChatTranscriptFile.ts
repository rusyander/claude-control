 ...
 * конец, начало не меняется, и стомегабайтный транскрипт стоит здесь ровно
 * столько же, сколько однострочный.
 */
export function readHeadRecords(path: string): Record[] {
  return parseLines(readHead(path));
}
