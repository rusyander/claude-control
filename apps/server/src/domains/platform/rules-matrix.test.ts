...
describe('список правил контура', () => {
  ...
  it('значение показывается словами, а конечный набор — списком допустимых', () => {
    ...
    expect(rows.find((row) => row.id === 'enterprise-platform_tools')?.value).toBe('web_search, code');
    expect(rows.find((row) => row.id === 'enable_thinking')?.value).toBe('включено');
    expect(rows.find((row) => row.id === 'enterprise-platform_tool_mode')?.options).toEqual([
      'loop',
      'single_turn',
    ]);
  });
...
describe('матрица конфликтов — по ячейке на строку', () => {
  it('инструменты платформы ⟷ прослойка: взаимное исключение', () => {
    const cell = ruleConflicts(PLATFORM, enterprise-platform, OURS).find((row) => row.id === 'tools');
    expect(cell?.level).toBe('exclusive');
    expect(cell?.ourRule).toBe('toolShim');
    expect(cell?.active).toBe(false);

    const both = ruleConflicts(withRules({ platformTools: ['web_search'] }), enterprise-platform, {
      ...OURS,
      toolShim: true,
    });
  ...
  it('сторона контура в ячейке — это строка ЕГО драйвера, а не имя платформа компании (аудит DRV-17)', () => {
    // Другой контур вправе назвать список инструментов по-своему: ячейка,
    // указывающая на «enterprise-platform_tools», подсветила бы строку, которой на его
    // карточке нет, и конфликт читался бы ни к чему не привязанным.
    const renamed = {
      ...enterprise-platform,
      controls: enterprise-platform.controls.map((control) =>
  ...
  it('подмена данных ⟷ наша защита: НЕ выбор из двух, а порядок слоёв (Р11)', () => {
    const cell = ruleConflicts(PLATFORM, enterprise-platform, { ...OURS, dlp: true }).find(
  ...
  it('сжатие истории ⟷ контрольные точки: предупреждение, и загорается по пробе', () => {
    const cell = ruleConflicts(PLATFORM, enterprise-platform, OURS).find((row) => row.id === 'compaction');
    expect(cell?.level).toBe('warning');
    expect(cell?.active).toBe(false);
    expect(
      ruleConflicts(PLATFORM, enterprise-platform, { ...OURS, managedContext: true }).find(
  ...
  it('гардрейлы ⟷ гейт промпта: информация о двух отказах', () => {
    const cell = ruleConflicts(PLATFORM, enterprise-platform, { ...OURS, promptGate: true }).find(
...
describe('нарушенное взаимное исключение', () => {
  it('называется, когда включены обе стороны', () => {
    const broken = brokenExclusion(withRules({ platformTools: ['web_search'] }), enterprise-platform);
    expect(broken?.id).toBe('tools');
  });

  it('молчит, когда включена одна', () => {
    expect(brokenExclusion(PLATFORM, enterprise-platform)).toBeUndefined();
    expect(
      brokenExclusion(withRules({ platformTools: ['web_search'] }, { toolShim: false }), enterprise-platform),
    ).toBeUndefined();
  });
});
...
describe('подмешивание управляемых правил в тело запроса', () => {
  it('кладёт только заданное: пустое правило поля не создаёт', () => {
    ...
    expect(applyManagedRules({ model: 'm' }, PLATFORM, enterprise-platform)).toEqual({
      model: 'm',
      tool_choice: 'none',
    });
  });
  ...
  it('пустой список инструментов выключает инструменты контура, а не молчит (ревью Т7, M2)', () => {
    ...
    expect(applyManagedRules({}, PLATFORM, enterprise-platform).tool_choice).toBe('none');
    // Список задан — выключать нечего, иначе заданные имена никогда бы не
    // сработали.
    expect(
      applyManagedRules({}, withRules({ platformTools: ['web_search'] }), enterprise-platform).tool_choice,
    ).toBeUndefined();
  ...
  it('имена инструментов и режим цикла уезжают вместе', () => {
    ...
    expect(body).toEqual({
      model: 'm',
      enterprise-platform_tools: ['web_search'],
      enterprise-platform_tool_mode: 'single_turn',
    });
  });

  it('режим цикла без инструментов не отправляется вовсе', () => {
    const body = applyManagedRules({}, withRules({ toolMode: 'single_turn' }), enterprise-platform);
  ...
  describe('размышления: путь на проводе и три состояния (аудит GW-09/DRV-09/MD-02)', () => {
    ...
    it('выключено уезжает значением false, а не отсутствием поля', () => {
      const body = applyManagedRules({}, withRules({ enableThinking: 'off' }), enterprise-platform);
      expect(body.chat_template_kwargs).toEqual({ enable_thinking: false });
      expect(body).not.toHaveProperty('enable_thinking');
    });

    it('по умолчанию поля нет вовсе: решает шаблон модели', () => {
      const body = applyManagedRules({}, withRules({ enableThinking: 'default' }), enterprise-platform);
    ...
    it('параметры шаблона клиента дополняются, а не затираются', () => {
      ...
      const body = applyManagedRules(client, withRules({ enableThinking: 'off' }), enterprise-platform);
    ...
    it('строка значения называет все три состояния словами', () => {
      const value = (enableThinking: PlatformRules['enableThinking']) =>
        platformRuleRows(withRules({ enableThinking }), enterprise-platform).find(
  ...
  it('исходное тело не переписывается на месте', () => {
    const body = { model: 'm' };
    applyManagedRules(body, withRules({ enableThinking: 'on' }), enterprise-platform);
    expect(body).toEqual({ model: 'm' });
  });
});
