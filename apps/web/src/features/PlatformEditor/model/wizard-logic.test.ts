...
describe('с чего начинается выбор целей', () => {
  it('у нового контура — ассистент панели', () => {
    // Единственный потребитель, работающий через контур полностью; пустой
    // список на этом шаге означал бы «подключено ни к чему».
    expect(initialTargets(undefined)).toEqual(['assistant']);
  });
  ...
  it('пустой выбор существующего читается как «не выбирали»', () => {
    const existing = { platform: { ...DRAFT, targets: [] } } as unknown as PlatformStatus;
    expect(initialTargets(existing)).toEqual(['assistant']);
  });
});
