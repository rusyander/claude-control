...
export const endpointProfileSchema = object({
  ...
  /**
   * Адрес генерации картинок этого эндпоинта — полный, вместе с путём
   * (`http://127.0.0.1:1234/v1/images/generations`). Пусто — пункт «Картинка» в
   * чате через этот профиль недоступен, и причина называется словами.
   *
   * Отдельное поле, а не путь, дописанный к `baseUrl` (решение В4): совместимый
   * с OpenAI сервер вправе не иметь этой ручки вовсе, а угаданный адрес даёт
   * 404 вместо честного «не объявлено» — тот же инвариант 13, что и с флагами
   * моделей. Вид API здесь всегда `openai-compat`: у `anthropic` и `google`
   * генерации картинок в API нет.
   */
  imagesUrl: string().default(''),
  /**
   * Профиль ПОРОЖДЁН контуром (Т3) и принадлежит ему: адрес всегда указывает на
   * локальный шлюз, руками он не редактируется и исчезает вместе с отключением
   * контура. Пусто — обычный профиль человека.
   *
   * Поле живёт здесь, а не отдельным списком «профили контуров», потому что
   * механизм применения ровно один: второй список означал бы вторую запись,
   * второй откат и второй шанс им разойтись.
   */
