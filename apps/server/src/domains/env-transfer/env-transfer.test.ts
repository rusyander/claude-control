...
describe('Перенос окружения: что не уезжает у Claude', () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(join(root, 'agentdeck', 'backups'), { recursive: true });
    writeFileSync(join(root, 'agentdeck', 'state.json'), '{"groups":[]}', 'utf8');
  });
  ...
  it('правила и скиллы едут, доступ, секреты, история и данные панели — нет', () => {
    ...
    expect(packed.some((path) => path.startsWith('agentdeck/'))).toBe(false);
