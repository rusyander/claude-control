 ...
 */
/**
 * Текст 401 живёт ОДНОЙ строкой на весь домен: его говорит и шлюз, и кнопка
 * «Проверить связь». Две копии разошлись бы на первой же правке, и человек
 * получал бы разный совет от двух кнопок про один и тот же отказ.
 */
...
const CODES: Record<number, { status: number; code: string; message: string }> = {
  ...
  // Поэтому панель называет ВСЕ причины: совет «перевыпустите ключ» человеку
  // с кончившимся бюджетом или с моргнувшей сверкой владельца отправлял бы
  // его чинить не то.
  401: {
    status: 401,
  ...
  // 402 — это НЕ бюджет ключа: контур отдаёт его с дневного лимита пользователя,
  // месячного команды или месячного инстанса. Какой именно — сказано в теле, и
  // его подставляет `budgetLevelOf`.
  402: {
    status: 402,
    code: 'billing_error',
...
export function bridgeUpstreamStatus(
  ...
  const known = CODES[status];
  const violations = status === 451 ? readViolations(payload) : [];
  ...
  // У 451 перечень нарушенного и есть причина: приписывать к нему чужую фразу
  // незачем, а вот названия проверок человеку нужны.
  const body =
    status === 451
      ? violations.length > 0
