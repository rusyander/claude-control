 ...
 *
 * Отдельно — случай, ради которого лента и разделяет «последнее сообщение» и
 * «открытый вопрос»: агент продолжает писать ПОСЛЕ вопроса (в пакетном режиме
 * вызов возвращается ошибкой сразу и ход не прерывает). Карточка обязана
 * остаться отвечаемой; закрывает её ответ человека, а не реплика агента.
 *
...
/** Ответ своими словами: ни один предложенный вариант не подошёл. */
const MY_ANSWER = 'Ответ своими словами про базу сравнения';
...
const MESSAGES = {
  messages: [
    ...
    /**
     * Агент продолжил говорить ПОСЛЕ вопроса — и это не редкость, а норма:
     * `AskUserQuestion` в пакетном режиме возвращается ошибкой сразу, так что
     * ход не прерывается. Пока карточка ждала «последнего сообщения ленты», она
     * теряла кнопки через несколько секунд после появления, посреди наполовину
     * заполненной формы. Вопрос закрывает ответ человека, а не реплика агента.
     */
    {
      id: 'm-after',
      role: 'assistant',
      blocks: [{ type: 'text', text: 'Жду ответа по вопросу выше — без него не двигаюсь.' }],
...
check(
  ...
  'последний вопрос отвечаем, хотя агент говорил после него',
);
check(
  await page.getByText('Жду ответа по вопросу выше').isVisible(),
  'реплика агента после вопроса на месте — именно она раньше и гасила карточку',
);
...
check(
  ...
  'до второго вопроса не дошли: вариантов нет вовсе',
);
check(
  (await live.getByText('Ответьте на предыдущий вопрос').count()) === 2,
  'следующие вопросы свёрнуты строкой и говорят, чего ждут',
);
...
// Свой вариант: варианты пишет модель, и ни один может не подходить. Отвечаем
// вторым вопросом своими словами — дальше этот текст живёт как обычный вариант.
check(
  (await live.getByRole('button', { name: 'Свой вариант' }).count()) === 1,
  'у активного вопроса есть свой вариант',
);
await live.getByRole('button', { name: 'Свой вариант' }).click();
await live.getByRole('textbox', { name: 'Свой вариант' }).fill(MY_ANSWER);
await live.getByRole('button', { name: 'Ответить так' }).click();
await page.waitForTimeout(200);
check(await live.getByText('вопрос 3 из 3').isVisible(), 'свой ответ закрыл вопрос, как и вариант');
check(
  (await live.getByText('Ответ своими словами про базу').count()) === 1,
  'свой ответ виден в карточке',
);
...
check(
  prompt.includes('Редактор: Monaco') &&
    prompt.includes(`База диффа: ${MY_ANSWER}`) &&
