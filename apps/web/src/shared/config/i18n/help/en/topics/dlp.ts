...
export const dlpEn: typeof dlpRu = {
  topic: {
    ...
    gateWhereText:
      'The script sits in the configuration hooks directory; with Claude its registration goes ' +
      'into settings.json as an ordinary hook, with another CLI the panel runs that very same ' +
      'script itself — there is no second, “for foreigners” script, it would drift from the ' +
      'first on the first rule change. It is visible in the Hooks section and can be disabled or deleted ' +
    ...
    guide: {
      ...
      gateForeign: '4. On a foreign CLI the gate works — with its boundary stated',
      gateForeignText:
        'The controls no longer go dead: such a CLI has no “prompt submitted” event, so the ' +
        'panel plays the hook itself inside its own run — same script, same rules, same journal. ' +
        'Nothing is written into the foreign CLI’s configuration. The card says exactly what ' +
        'follows from that: the same CLI started by hand from a terminal will not see the gate. ' +
        'The proxy has no such boundary: it is on the wire, not inside a CLI.',
    },
  ...
  shots: {
    ...
    gate: {
      ...
      '04-foreign-cli':
        'The same card on a foreign CLI: the controls work, and the line beside them names the boundary — runs through the panel only',
    },
  },
