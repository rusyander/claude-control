/**
 * Схема путеводителя «Эндпоинты». Источник — этот генератор, `.drawio` — его вывод:
 *
 *   node docs/diagrams/endpoints-guide/generate.mjs <абсолютный путь>.drawio
 *
 ...
 *
 * Страница одна. Главное, что она объясняет: профиль живёт у панели, а у CLI
 * появляется переменная окружения — и только после кнопки «Применить»; уже
 * запущенная сессия CLI о ней не узнает.
 */
...
// ─── Страница: где оказывается адрес ───────────────────────────────────────

const p1 = newPage('Где оказывается адрес', 'p1');
heading(
  p1,
  60,
  30,
  'Профиль у панели, переменная у CLI',
  'Профиль работает ровно до кнопки «Применить»: пока её не нажали, ни один CLI об этом адресе не знает.',
  1240,
);
...
const profile = card(p1, 'e:profile', AX, 150, AW, {
  pal: C.panel,
  icon: 'shape=card',
  iconText: 'профиль',
  title: 'Профиль в панели',
  lines: [
    'Название, вид API, адрес и модель. Лежат открыто в agentdeck/state.json → endpointProfiles: секрета в них нет.',
    'Вид API решает и смысл адреса, и то, какие CLI профиль примут.',
  ],
});

const token = card(p1, 'e:token', AX, profile.y + profile.h + 55, AW, {
  pal: C.store,
  icon: 'shape=note;size=14',
  iconText: 'enc',
  title: 'Токен — отдельно и зашифрованно',
  lines: [
    'agentdeck/provider-keys.enc под ключом endpoint: плюс идентификатор профиля. Наружу отдаётся только маска; в чужой конфиг уходит исключительно по явной галочке, с предупреждением.',
  ],
});

card(p1, 'e:probe', AX, token.y + token.h + 55, AW, {
  pal: C.check,
  icon: 'shape=hexagon',
  iconText: 'проба',
  title: '«Проверить связь»',
  lines: [
    'Запрос за списком моделей — не за генерацией. Ничего не записывает ни в один CLI и ничего не стоит.',
  ],
});

const apply = card(p1, 'e:apply', BX, 150, BW, {
  pal: C.panel,
  icon: 'shape=process',
  iconText: 'применить',
  title: 'Кнопка у принимающего CLI',
  lines: [
    'До нажатия видно, какая переменная и в какой файл уйдёт. Перед записью снимается копия файла — как при любой другой правке конфигурации.',
  ],
});

const cli = card(p1, 'e:cli', BX, apply.y + apply.h + 55, BW, {
  pal: C.cli,
  icon: 'shape=process',
  iconText: 'CLI',
  title: 'Файл окружения CLI',
  lines: [
    'У Claude Code — ключ env в settings.json; у Qwen Code — ~/.qwen/.env; у Aider — set-env в ~/.aider.conf.yml.',
    'Профиль ДОПОЛНЯЕТ окружение: существующие переменные переносятся целиком, одноимённые перекрываются.',
  ],
});

card(p1, 'e:restart', BX, cli.y + cli.h + 55, BW, {
  pal: C.warn,
  icon: 'shape=card',
  iconText: 'старт',
  title: 'CLI читает переменные при старте',
  lines: [
    'Запись мгновенна, но уже запущенная сессия о ней не узнает. Перезапуск CLI — единственный способ, и панель делает его не за вас.',
  ],
});

const assistant = card(p1, 'e:assistant', CX, 150, CW, {
  pal: C.panel,
  title: 'Помощник панели — отдельно',
  lines: [
    'Свой переключатель в том же блоке. Выбран профиль — подсказки в формах идут по этому адресу, минуя и облако, и CLI провайдера. Ничего никуда не записывается.',
  ],
});

const no = card(p1, 'e:no', CX, assistant.y + assistant.h + 55, CW, {
  pal: C.warn,
  title: 'Почему CLI «не принимает»',
  lines: [
    'Вид API не тот, что понимает этот CLI.',
    'Переменной адреса у него не задокументировано — адрес правится только руками в его конфиге.',
    'Своего файла переменных окружения у него нет вовсе — писать некуда.',
  ],
});

card(p1, 'e:not', CX, no.y + no.h + 55, CW, {
  pal: C.warn,
  title: 'Чего профиль не делает',
  lines: [
    'Не поднимает модель по адресу, не смотрит, ЧТО уходит в запросе, и не встаёт на его пути: панель только пишет адрес, дальше CLI ходит сам.',
  ],
});

link(p1, 'e:profile', 'e:apply', 'адрес и модель', {
  colour: FLOW.fwd,
  exit: [1, 0.3],
  entry: [0, 0.4],
});
link(p1, 'e:token', 'e:probe', 'подставляется в пробу', {
  colour: FLOW.data,
  exit: [0.5, 1],
  entry: [0.5, 0],
});
link(p1, 'e:probe', 'e:profile', 'список моделей в поле', {
  colour: FLOW.back,
  exit: [1, 0.2],
  entry: [1, 0.9],
});
link(p1, 'e:apply', 'e:cli', 'запись + копия файла', {
  colour: FLOW.fwd,
  exit: [0.5, 1],
  entry: [0.5, 0],
});
link(p1, 'e:cli', 'e:restart', 'увидит при старте', {
  colour: FLOW.fwd,
  exit: [0.5, 1],
  entry: [0.5, 0],
});
link(p1, 'e:profile', 'e:assistant', 'тот же адрес, другой переключатель', {
  colour: FLOW.data,
  exit: [1, 0.1],
  entry: [0, 0.3],
});
link(p1, 'e:apply', 'e:no', 'кому кнопка недоступна', {
  colour: FLOW.back,
  dashed: true,
  exit: [1, 0.7],
  entry: [0, 0.4],
...
legend(p1, 60, bottomOf(p1) + 60, [
  ...
  { caption: 'границы и отказы', fill: C.warn.tint, stroke: C.warn.line },
