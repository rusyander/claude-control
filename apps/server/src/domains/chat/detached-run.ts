 ...
 * текста ответа и расхода (они в транскрипте, лента читает его сама), а значит
 * и повода для продолжений и конвейера — об этом реестр говорит лентой.
 */
...
export const ADOPTED_NOTICE =
  'Панель перезапускалась: прогон подхвачен без потока вывода. Ответ агента читается из транскрипта, запросы прав работают; продолжение и конвейер после этого прогона не заводятся.';
export const DETACHED_DONE_NOTICE =
  'Процесс прогона завершился. Ответ — в транскрипте; продолжение и конвейер после подхваченного прогона панель не заводит.';
