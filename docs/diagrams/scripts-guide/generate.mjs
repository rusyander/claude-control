/**
 * Схема путеводителя «Скрипты». Источник — этот генератор, `.drawio` — его вывод:
 *
 *   node docs/diagrams/scripts-guide/generate.mjs <абсолютный путь>.drawio
 *
 ...
 *
 * Страница одна, и она про единственную нетривиальную вещь раздела: как
 * получается пометка «используется». Привязка транзитивна — общий модуль, не
 * названный ни в одной команде хука, всё равно считается привязанным, — и без
 * рисунка это читается как ошибка панели. Контракт со stdin/stdout на схему не
 * вынесен намеренно: он есть полосой в самом документе.
 */
...
// ─── Страница: как получается пометка «используется» ───────────────────────

const p1 = newPage('Откуда пометки', 'p1');
heading(
  p1,
  60,
  30,
  'Как считается «используется»',
  'Панель идёт от команд хуков к файлам и дальше по цепочке импортов. Всё, до чего не дотянулась, помечено «не привязан».',
  1200,
);
...
const settings = card(p1, 'u:settings', AX, 150, AW, {
  pal: C.store,
  icon: 'shape=note;size=14',
  iconText: 'json',
  title: 'Команды хуков',
  lines: [
    'Берутся из settings.json И settings.local.json: хук из личного файла срабатывает так же, и не учесть его значило бы соврать.',
    'В команде ищется имя файла — с этого и начинается вся цепочка.',
  ],
});

const folder = card(p1, 'u:folder', AX, settings.y + settings.h + 55, AW, {
  pal: C.store,
  icon: 'shape=folder',
  iconText: 'hooks',
  title: '~/.claude/hooks/ целиком',
  lines: [
    'Вместе с вложенными папками. Скриптом считается файл с одним из девяти расширений: .mjs .cjs .js .ts .mts .cts .sh .ps1 .py.',
    'Файл с другим расширением раздел не покажет вовсе, а файл вне этой папки в список не попадёт.',
  ],
});

card(p1, 'u:outside', AX, folder.y + folder.h + 55, AW, {
  pal: C.note,
  title: 'Чего панель проследить не может',
  lines: [
    'Пакет из node_modules и путь, собранный в переменной: по ним цепочка обрывается.',
    'Такой файл будет помечен «не привязан», хотя работает. Пометка — повод проверить, а не приговор.',
  ],
});

const direct = card(p1, 'u:direct', BX, 150, BW, {
  pal: C.check,
  icon: 'shape=hexagon',
  iconText: '1',
  title: 'Имя файла названо в команде хука',
  lines: [
    'Прямая привязка. Именно её создаёт панель, когда заводит скрипт из формы хука и подставляет абсолютный путь.',
    'Это первый и очевидный источник пометки «Используется».',
  ],
});

const chain = card(p1, 'u:chain', BX, direct.y + direct.h + 55, BW, {
  pal: C.check,
  icon: 'shape=hexagon',
  iconText: '→',
  title: 'Относительные import и require, по цепочке',
  lines: [
    'От привязанного файла — ко всему, что он тянет, и дальше вглубь, а не на один шаг.',
    'Поэтому общий модуль вроде shared/input.mjs, которого нет ни в одной команде хука, забытым не считается.',
  ],
});

card(p1, 'u:tests', BX, chain.y + chain.h + 55, BW, {
  pal: C.group,
  icon: 'shape=hexagon',
  iconText: 'test',
  title: 'Тесты и фикстуры — отдельно',
  lines: [
    'Папка tests/ и имена вида *.test.* или *.spec.* помечаются «Тест».',
    'В счётчик непривязанных они не входят: тест и не должен вызываться хуком, а без отдельной пометки он каждый раз читался бы как забытый файл.',
  ],
});

const used = card(p1, 'u:used', CX, 150, CW, {
  pal: C.group,
  icon: 'shape=card',
  iconText: 'ok',
  title: 'Пометка «Используется»',
  lines: [
    'Всё, до чего дотянулась цепочка. Удаление такого файла панель отдельно предупреждает: хук останется в settings.json и перестанет работать молча.',
  ],
});

const orphan = card(p1, 'u:orphan', CX, used.y + used.h + 55, CW, {
  pal: C.warn,
  icon: 'shape=card',
  iconText: '?',
  title: 'Пометка «Не привязан»',
  lines: [
    'Остальное, и подзаголовок раздела считает такие файлы отдельным числом.',
    'Обычно это либо забытая привязка, либо остаток от удалённого хука: сам файл при удалении хука никуда не девается.',
  ],
});

card(p1, 'u:bind', CX, orphan.y + orphan.h + 55, CW, {
  pal: C.panel,
  title: 'Как привязать',
  lines: [
    'Созданный здесь файл не запускается сам никогда. Нужен хук в разделе «Хуки», указывающий на этот скрипт.',
    'И наоборот: чтобы временно отключить скрипт, достаточно выключить хук — файл останется на месте.',
  ],
});

link(p1, 'u:settings', 'u:direct', 'имя файла в команде', {
  colour: FLOW.data,
  exit: [1, 0.3],
  entry: [0, 0.4],
});
link(p1, 'u:folder', 'u:direct', 'все файлы папки', {
  colour: FLOW.data,
  exit: [1, 0.2],
  entry: [0, 0.85],
});
link(p1, 'u:direct', 'u:chain', 'дальше по импортам', {
  colour: FLOW.fwd,
  exit: [0.5, 1],
  entry: [0.5, 0],
});
link(p1, 'u:direct', 'u:used', 'привязан', { colour: FLOW.fwd, exit: [1, 0.3], entry: [0, 0.6] });
link(p1, 'u:chain', 'u:used', 'тоже привязан', {
  colour: FLOW.fwd,
  exit: [1, 0.2],
  entry: [0, 0.9],
});
link(p1, 'u:tests', 'u:orphan', 'в счётчик не входят', {
  colour: FLOW.back,
  dashed: true,
  exit: [1, 0.4],
  entry: [0, 0.15],
});
// Коридор между рядами средней колонки: прямая линия отсюда направо пересекла бы
// карточку про импорты по тексту, а связь «файл, до которого не дотянулись»
// нужна именно от папки.
const corridorY = chain.y + chain.h + 27;
link(p1, 'u:folder', 'u:orphan', 'не дотянулись', {
  colour: FLOW.back,
  dashed: true,
  exit: [0.5, 1],
  entry: [0.25, 1],
  points: [
    { x: AX + AW / 2, y: corridorY },
    { x: CX + CW * 0.25, y: corridorY },
  ],
});
link(p1, 'u:orphan', 'u:bind', 'что с этим делать', {
  colour: FLOW.human,
  exit: [0.7, 1],
  entry: [0.7, 0],
});
...
legend(p1, 60, bottomOf(p1) + 60, [
  ...
  { caption: 'как считает панель', fill: C.check.tint, stroke: C.check.line },
  { caption: 'привязано', fill: C.group.tint, stroke: C.group.line },
  { caption: 'повод проверить', fill: C.warn.tint, stroke: C.warn.line },
  { caption: 'что делать', fill: C.panel.tint, stroke: C.panel.line },
  { caption: 'пояснение', fill: C.note.tint, stroke: C.note.line },
  { caption: 'кто что читает', edgeColour: FLOW.data },
  { caption: 'цепочка привязки', edgeColour: FLOW.fwd },
  { caption: 'действие человека', edgeColour: FLOW.human },
  { caption: 'не привязано', edgeColour: FLOW.back, dashed: true },
]);
