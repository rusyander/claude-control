 ...
 *
 * Список ЗАКРЫТ и закреплён тестом (`emit.test.ts` §«список невозимых видов
 * закрыт…»): он обязан сокращаться правкой, а не молчанием. Пока вид числится
 * здесь, его записи в план не попадают вовсе — строка «исход неизвестен» была бы
 * хуже отсутствия строки, потому что читалась бы как приговор.
 *
 * Дописать сюда вид — единственный способ убрать записи из плана БЕЗ красного
 * теста, поэтому цена такой правки — правка теста рядом, на глазах у читающего
 * дифф. Всякий другой вид без эмиттера план роняет, назвав запись.
 */
