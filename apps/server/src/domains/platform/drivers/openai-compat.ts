...
export const openAiCompatDriver: PlatformDriver = {
    ...
    const capabilities: PlatformCapabilityFinding[] = [
      {
        ...
        detailCode: 'capability-models-gateway-listed',
        count: models.length,
      },
      ...
      ...UNDECLARED.map((id): PlatformCapabilityFinding => ({
        ...
        detailCode: 'capability-undeclared-by-gateway',
        compromise: 'probe-guess',
      })),
      // Рисование читается из каталога тем же кодом, что и у платформы компании: поля
   ...
   * отбрасываются» значило бы перенести на чужой шлюз свойство платформы компании.
   */
  requestFields: [],
   ...
   * платформе компании, проверенное в её исходниках, и у произвольного шлюза причина
