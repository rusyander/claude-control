 ...
 *
 * Разделитель `/` даже на Windows — по той же причине, что в `hook-command.ts`:
 * обратная косая в строке команды для всякой оболочки семейства sh есть
 * экранирование, а не разделитель. Метка в команду НЕ дописывается: `#` —
 * комментарий для sh, но не для `cmd.exe`, и хук с таким хвостом на Windows не
 * запустился бы вовсе. Узнаётся переходник по метке ВНУТРИ файла.
 *
 * КАВЫЧКИ — ТОЛЬКО ПОД ПРОБЕЛ, и это тот же счёт, что в `hook-command.ts`.
 * Раньше они стояли всегда, и живая приёмка 22.09.2026 (`qwen 0.24.3`) показала
 * цену: свою команду хука эта цель отдаёт `cmd.exe /d /s /c` ОДНИМ элементом
 * argv, node экранирует внутренние кавычки как `\"`, и до скрипта путь доезжает
 * ВМЕСТЕ с ними — «Cannot find module '<cwd>\"C:\...\"'». Хук при этом не падает
 * громко: цель считает его ошибкой и вызов РАЗРЕШАЕТ, то есть перенесённый
 * запрет молча снимается (инвариант 6). Без кавычек тот же путь исполняется и
 * `cmd.exe`, и bash, и sh.
 *
 * Путь С ПРОБЕЛОМ остаётся в кавычках: без них его разберёт на части любая
 * оболочка. Для цели, зовущей хук через `cmd.exe`, такой путь непроходим в
 * принципе — оба написания ломаются, — и честнее оставить рабочее для всех
 * прочих, чем сломать ещё и их.
 */
export function hookShimCommand(path: string): string {
  const spelled = path.split('\\').join('/');
  return spelled.includes(' ') ? `node "${spelled}"` : `node ${spelled}`;
}
...
export function readHookShimCommand(command: string): string | undefined {
  // ОБА написания, и не про запас: кавычки панель теперь ставит только под
  // пробел (см. `hookShimCommand`), а в конфиге цели могла остаться команда,
  // записанная прежней версией. Знай чтение одну форму — свой же переходник
  // панель перестала бы узнавать и записала бы его в канон как хук человека.
