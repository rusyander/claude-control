 ...
 *
 * Список ПОЛНЫЙ по словарю канона, и полнота закреплена тестом
 * (`subscribe.test.ts` §«порядок экрана ПОЛОН по словарю канона»): по нему
 * пересобирается не только вид экрана, но и набор подписанных слоёв. Вид,
 * забытый здесь, уедет в конец (`kindOrder` возвращает длину списка) и из
 * подписки не пропадёт — потерять запись значило бы показать среду, которой нет.
 */
