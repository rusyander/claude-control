...
export const EXCLUDED_DIRS = new Set([
  ...
  BRAND_SLUG, // данные самой панели: группы, связи, ключи
  LEGACY_BRAND_SLUG, // они же под прежним именем — резервная копия после переезда
 ...
 * аккаунтом и идентификаторами машины (не настройка). Берём только
 * ключи-настройки, а на новой машине вливаем их в существующий файл, а не
 * поверх него.
 *
 * Правило узнаёт файл и по РОЛИ места, и по ИМЕНИ: при домашнем каталоге файл
 * лежит рядом с `~/.claude` и приходит отдельным местом с ролью `mcp`, а при
 * заданном каталоге конфигурации — ВНУТРИ него (правило самого CLI,
 * `claude-paths.ts → resolveMcpConfig`), и тогда его забирает обход корня с
 * ролью `config`. По одной роли он уехал бы целиком — с историей и аккаунтом.
 */
export const PARTIAL_JSON: Record<string, { role: string; name: string; keys: string[] }> = {
  claude: { role: 'mcp', name: '.claude.json', keys: ['mcpServers'] },
};
