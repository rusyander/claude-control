...
export const streamRun = (
  ...
  new Promise((resolve) => {
    ...
    // Заголовки — сразу, а не с первым кадром: Node держит их до первой
    // записи, и у отправки в идущий разговор (буфер пуст, агент ещё молчит)
    // первой записью был пинг через 10 с. Всё это время клиент не знал, что
    // сообщение принято (живой прогон 25.09: «отправлен» через 10 с).
    reply.raw.flushHeaders();
    // Прокси между нами и браузером (Vite на :8888, Tailscale для телефона) тоже
    // держат заголовки до первого куска тела: сам сервер отвечал за 0,2 с, а
    // через Vite — за 10 с, с первым пингом, и всё это время отправленный текст
    // висел в поле ввода. Пустой комментарий сразу проталкивает ответ насквозь.
