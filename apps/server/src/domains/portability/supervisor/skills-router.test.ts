...
describe('скилл, названный в тексте', () => {
  ...
  it('имя внутри РУССКОГО слова скиллом не считается', () => {
    // Граница слова, написанная как «не латинская буква», считала кириллицу
    // границей: «переразборку» попадало под скилл «разбор», и человек получал
    // тело скилла, которого не звал.
    const russian = [entry('разбор', 1)];
    expect(matchSkillName('отдай на переразборку', russian)).toBeUndefined();
    expect(matchSkillName('нужен разбор этой правки', russian)).toBe('разбор');
  });
