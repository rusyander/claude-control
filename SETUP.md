...
Everything needed to get AgentDeck running, plus what to do when it does not.
...
This is the folder AgentDeck edits. Knowing where it is makes most problems obvious.
...
└── agentdeck/        created by this panel
...
```bash
export AGENTDECK_KEYCHAIN_SERVICE="Claude Code-credentials"
pnpm dev
```
...
**What happens to what you enter.** It is saved to `~/.agentdeck/credentials.json` with `600` permissions. It is never sent back to the browser: the panel reports the source, never the token itself. You can also create that file by hand without the UI — the panel picks it up on the next read.

**Source order** (first match wins):

1. `~/.agentdeck/credentials.json` — set by hand
...
```
AgentDeck API: http://127.0.0.1:5178
...
Every write is backed up first. Look in `~/.claude/agentdeck/backups/` for `<name>.<timestamp>.bak` and copy the version you want back over the original. The last ten copies of each file are kept — otherwise the directory would grow without limit, and it holds copies of the secrets file too.
...
**Reverting a single edit** — easiest from the panel: **Settings → Backups**, the "Restore" button next to the copy you want. The file is replaced whole, and the state before the restore is saved as a fresh copy — so even a restore can be undone. The copies still live in `~/.claude/agentdeck/backups/` (last ten per file) and can be copied back by hand.

**Resetting the panel itself** — delete `~/.claude/agentdeck/state.json`. Groups, automations and panel settings go; your Claude Code configuration is untouched.

**Removing the panel completely:**

```bash
rm -rf ~/.agentdeck          # chat working folders and sandboxes
rm -rf ~/.claude/agentdeck   # panel state and backups
```
...
> [!TIP]
> Before deleting, check `~/.agentdeck/chats/` for artifacts a chat produced that you still want, and keep `~/.claude/agentdeck/backups/` until you are sure you need nothing from it.
