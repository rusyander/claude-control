...
if (existsSync(panelFile)) {
  ...
  if (found) {
    ...
    warn(
      ...
      'Обычный чат и все разделы работают — не заработает только песочница. Три пути: войти командой «claude» в терминале; задать имя записи в AGENTDECK_KEYCHAIN_SERVICE; либо ввести доступ вручную в Настройках панели. Подробности в SETUP.ru.md → Различия между системами.',
    );
  }
} else {
  warn(
