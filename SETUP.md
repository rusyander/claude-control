...
Every write is backed up first. Look in `~/.claude/agentdeck/backups/` for `<name>.<timestamp>.bak` and copy the version you want back over the original.
...
**Reverting a single edit** — restore from `~/.claude/agentdeck/backups/`, where every write left a timestamped copy.
