...
if (prompt === undefined) {
  ...
  warn('AgentDeck: гейт не разобрал ввод хука, промпт НЕ проверен.');
}
...
try {
  ...
  warn('AgentDeck: правила гейта не прочитаны, промпт НЕ проверен.');
}
...
if (blocked) {
  ...
  process.stderr.write(
    'AgentDeck: промпт не отправлен — сработали правила защиты данных: ' + names +
      '. Уберите эти данные из текста или отправьте их иначе.',
  );
  process.exit(2);
}

warn('AgentDeck: в промпте найдено то, что попадает под правила (' + names + '). Отправлено как есть.');
`;
}
