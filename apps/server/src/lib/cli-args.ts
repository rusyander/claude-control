 ...
 *
 * Грамматика идёт ЗА каталогом контура, а не за вендорными именами: имя панель
 * не придумывает, а предлагает человеку то, что прислал контур, и молча
 * выброшенное имя означает прогон с моделью, которую CLI выберет сам, — без
 * единой строки на экране. Поэтому здесь `:` и `/` (`qwen2.5:7b`,
 * `vendor/gpt-4o`), `@` (версия модели у Vertex: `gemini-1.5-pro@002`) и длина
 * до 200 знаков (ARN профиля вывода у Bedrock длиннее 64). Оболочке cmd.exe
 * ничто из этого командой не является: опасны `& | < > ^ " % ( ) ; , !` и
 * пробел — их здесь по-прежнему нет.
 */
