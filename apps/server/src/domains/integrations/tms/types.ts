...
/**
 * Тест-менеджмент в Jira — одним интерфейсом на две системы.
 *
 * Zephyr Scale и Xray решают одну задачу и делают это по-разному: у Zephyr свой
 * REST поверх ключа проекта, у Xray — пара «клиент+секрет», JWT и импорт
 * результатов одним документом. Наружу это не протекает: панель забирает кейсы
 * и отправляет прогон, а какая из двух систем на том конце — вопрос настройки.
 *
...
export interface TmsClient {
  /** Как система называется человеку: «Zephyr Scale», «Xray». */
  title: string;
  /**
