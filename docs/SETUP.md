...
Everything needed to get AgentDeck running, and what to do when it does not.
...
What you enter is stored in `~/.agentdeck/credentials.json` with `600` permissions and is
never returned to the browser — the panel shows only the source. The same file can be created by
hand.

**Source order** (first hit wins): `~/.agentdeck/credentials.json` (manual) →
...
- **Interactive OAuth is needed.** Network servers have an "Authorize" button: the token is stored
  in `agentdeck/mcp-oauth.json` (mode 600) and refreshed automatically. If no window opened,
...
into `agentdeck/pricing-cache.json`; the rate is matched to the exact model version from the
...
Every write is preceded by a backup: `~/.claude/agentdeck/backups/`,
...
4. **`pnpm mobile`** — builds and installs the app on a connected phone or emulator (needs the
   ...
   `agentdeck.apk` in the repository root, replacing the previous one. Android 12 or newer.
...
backups live in `~/.claude/agentdeck/backups/` (last ten per file) and can still be copied by
hand.

**Reset the panel itself** — delete `~/.claude/agentdeck/state.json`: groups, automations and
...
> [!TIP]
> Before deleting, look into `~/.agentdeck/chats/` for artifacts you may still want, and keep
