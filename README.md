...
AgentDeck answers all three. It gives the configuration a shape you can see, an off switch that does not destroy anything, and a sandbox where you can test a change against the real Claude Code before it becomes your daily setup.
...
**Writes are defensive.** Every write goes through a backup into `~/.claude/agentdeck/backups/` and lands atomically (write to a temp file, then rename), so an interrupted save cannot leave you with a half-written `settings.json`.
...
Any key you do enter is stored encrypted (AES-256-GCM) in `agentdeck/provider-keys.enc`; the passphrase is a machine-local file with `0600` permissions. Only a mask like `sk-…1234` ever leaves the server. Keys never reach the logs, the backups, the history feed, the global search or the bundle export — there is a dedicated test that walks every provider to prove it.
...
- **The application writes nothing inside the repository.** Every write path resolves under `~/.claude` or `~/.agentdeck`; there are no writes relative to `process.cwd()` in the server at all.
